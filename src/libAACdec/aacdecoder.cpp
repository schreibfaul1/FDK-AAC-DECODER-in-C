...
void CAacDecoder_SyncQmfMode(HANDLE_AACDECODER self) {
	...
	if(IS_USAC(self->streamInfo.aot)) { self->qmfModeCurr = MODE_HQ; }

	if(self->qmfModeCurr == NOT_DEFINED) {
		if((IS_LOWDELAY(self->streamInfo.aot) && (self->flags[0] & AC_MPS_PRESENT)) ||
		   ((self->streamInfo.aacNumChannels == 1) &&
			((CAN_DO_PS(self->streamInfo.aot) && !(self->flags[0] & AC_MPS_PRESENT)) ||
			 (IS_USAC(self->streamInfo.aot))))) {
			self->qmfModeCurr = MODE_HQ;
		}
		else { self->qmfModeCurr = MODE_LP; }
	}
	...
	if(self->mpsEnableCurr) {
		...
		switch(mpegSurroundDecoder_SetParam((CMpegSurroundDecoder *)self->pMpegSurroundDecoder,
			...
			case MPS_INVALID_PARAMETER: { /* Only one mode supported. Find out which
				...
				if(((mpsCaps & CAPF_MPS_LP) && (self->qmfModeCurr == MODE_LP)) ||
				   ((mpsCaps & CAPF_MPS_HQ) &&
					(self->qmfModeCurr == MODE_HQ))) { /* MPS decoder does support the requested mode. */
					break;
				}
			}
				/* fall through */;
			default:
				if(self->qmfModeUser == NOT_DEFINED) {
					...
					self->qmfModeCurr = (self->qmfModeCurr == MODE_LP) ? MODE_HQ : MODE_LP;
				}
				else {
...
static int32_t CAacDecoder_GetELChannels(MP4_ELEMENT_ID_t type, uint8_t usacStereoConfigIndex) {
	...
	switch(type) {
		case ID_USAC_CPE:
			if(usacStereoConfigIndex == 1) { el_channels = 1; }
			else { el_channels = 2; }
			break;
		case ID_CPE:
			el_channels = 2;
...
/*!
  ...
  \return  Error code
*/
static AAC_DECODER_ERROR_t CAacDecoder_AncDataReset(CAncData *ancData) {
	int32_t i;
	for(i = 0; i < 8; i++) { ancData->offset[i] = 0; }
...
/*!
  ...
  \return  Error code
*/
static AAC_DECODER_ERROR_t CAacDecoder_AncDataParse(CAncData *ancData, HANDLE_FDK_BITSTREAM hBs, const int32_t ancBytes) {
	AAC_DECODER_ERROR_t error = AAC_DEC_OK;
	int32_t               readBytes = 0;
	...
	if(ancData->buffer != NULL) {
		if(ancBytes > 0) {
			...
			int32_t offset = ancData->offset[ancData->nrElements];

			if((offset + ancBytes) > ancData->bufferSize) { error = AAC_DEC_TOO_SMALL_ANC_BUFFER; }
			else if(ancData->nrElements >= 8 - 1) { error = AAC_DEC_TOO_MANY_ANC_ELEMENTS; }
			else {
				int32_t i;
...
/*!
  ...
  \return  Error code
*/
static AAC_DECODER_ERROR_t CDataStreamElement_Read(HANDLE_AACDECODER self, HANDLE_FDK_BITSTREAM bs,
	...
	*elementInstanceTag = FDKreadBits(bs, 4);
	...
	if(count == 255) { count += FDKreadBits(bs, 8); /* EscCount */ }
	dseBits = count * 8;

	if(dataByteAlignFlag) { FDKbyteAlign(bs, alignmentAnchor); }

	dataStart = (int32_t)FDKgetValidBits(bs);
...

	{
		...
		FDKpushBack(bs, dataStart - (int32_t)FDKgetValidBits(bs));
...

	{
		...
		if(error == AAC_DEC_OK && dmxErr != PCMDMX_OK) { error = AAC_DEC_UNKNOWN; }
	}

	/* Move to the very end of the element. */
	FDKpushBiDirectional(bs, (int32_t)FDKgetValidBits(bs) - dataStart + (int32_t)dseBits);
...
/*!
  ...
  \return  Error code
*/
AAC_DECODER_ERROR_t CAacDecoder_PrepareCrossFade(const int16_t *pTimeData, int16_t **pTimeDataFlush,
...
/*!
  ...
  \return  Error code
*/
AAC_DECODER_ERROR_t CAacDecoder_ApplyCrossFade(int16_t *pTimeData, int16_t **pTimeDataFlush, const int32_t numChannels,
...
AAC_DECODER_ERROR_t CAacDecoder_PreRollExtensionPayloadParse(HANDLE_AACDECODER self, uint32_t *numPrerollAU,
	...
	uint32_t  independencyFlag;
	uint32_t  extPayloadPresentFlag;
	uint32_t  useDefaultLengthFlag;
	...
	auStartAnchor = (int32_t)FDKgetValidBits(hBs);
	if(auStartAnchor <= 0) {
	...
	if((self->streamInfo.numChannels == 0) && !implicitExplicitCfgDiff && (self->flags[0] & AC_USAC)) {
	...
	for(i = 0; i < *numPrerollAU; i++) {
		...
		if(prerollAULength[i] > 0) {
			prerollAUOffset[i] = auStartAnchor - (int32_t)FDKgetValidBits(hBs);
...
/*!
  ...
  \return  Error code
*/
static AAC_DECODER_ERROR_t CAacDecoder_ExtPayloadParse(HANDLE_AACDECODER self, HANDLE_FDK_BITSTREAM hBs, int32_t *count,
	...
	*count -= 4;
	...
	/* For ELD, the SBR signaling is explicit and parsed in
	   ...
	   present. */
	if((self->flags[0] & AC_ELD) && ((extension_type == EXT_SBR_DATA_CRC) || (extension_type == EXT_SBR_DATA))) {
	...
	switch(extension_type) {
		...
		case EXT_UNI_DRC: {
			...
			int32_t                nBitsRemaining = FDKgetValidBits(hBs);
			int32_t                readBits;
			...
			readBits = (int32_t)nBitsRemaining - (int32_t)FDKgetValidBits(hBs);
			if(readBits > *count) { /* Read too much. Something went wrong! */
				error = AAC_DEC_PARSE_ERROR;
			}
			*count -= readBits;
		...
		case EXT_SAC_DATA:
			...
			{
				int32_t err, mpsSampleRate, mpsFrameSize;
				...
				if(self->streamInfo.extSamplingRate != 0) {
					...
					mpsFrameSize = self->streamInfo.aacSamplesPerFrame *
								   (self->streamInfo.extSamplingRate / self->streamInfo.aacSampleRate);
				}
				else {
				...
				/* Setting of internal MPS state; may be reset in
				   ...
				   defined qmfMode */
				if(!(self->flags[0] & (AC_USAC | AC_RSVD50 | AC_ELD))) { self->mpsEnableCurr = self->mpsEnableUser; }
				if(self->mpsEnableCurr) {
					...
					err = mpegSurroundDecoder_Parse((CMpegSurroundDecoder *)self->pMpegSurroundDecoder, hBs, count,
													self->streamInfo.aot, mpsSampleRate, mpsFrameSize,
													self->flags[0] & AC_INDEP);
					if(err == MPS_OK) { self->flags[0] |= AC_MPS_PRESENT; }
					else { error = AAC_DEC_PARSE_ERROR; }
				}
		...
		case EXT_SBR_DATA:
			if(IS_CHANNEL_ELEMENT(previous_element)) {
				...
				sbrError = sbrDecoder_InitElement(self->hSbrDecoder, self->streamInfo.aacSampleRate,
												  self->streamInfo.extSamplingRate, self->streamInfo.aacSamplesPerFrame,
												  self->streamInfo.aot, previous_element, elIndex,
				...
				if(sbrError == SBRDEC_OK) {
					sbrError =
						sbrDecoder_Parse(self->hSbrDecoder, hBs, self->pDrmBsBuffer, self->drmBsBufferSize, count,
										 *count, crcFlag, previous_element, elIndex, self->flags[0], self->elFlags);
					/* Enable SBR for implicit SBR signalling but only if no severe error
					 * happend. */
					if((sbrError == SBRDEC_OK) || (sbrError == SBRDEC_PARSE_ERROR)) { self->sbrEnabled = 1; }
				}
				else {
				...
				*/
				if(fIsFillElement) {
					...
					*count = 0;
					...
					if(sbrError != SBRDEC_OK) { self->frameOK = 0; }
				}
			}
			else { error = AAC_DEC_PARSE_ERROR; }
			break;
		...
		case EXT_DATA_LENGTH:
			if(!fIsFillElement              /* Makes no sens to have an additional length in a
											   fill ...   */
			   && (self->flags[0] & AC_ER)) /* ... element because this extension payload type was ... */
			{                               /* ... created to circumvent the missing length in ER-Syntax. */
				int32_t bitCnt, len = FDKreadBits(hBs, 4);
				*count -= 4;

				if(len == 15) {
					int32_t add_len = FDKreadBits(hBs, 8);
					*count -= 8;
					len += add_len;
				...
				if((EXT_PAYLOAD_TYPE_t)FDKreadBits(hBs, 4) == EXT_DATA_LENGTH) {
					...
					error = CAacDecoder_ExtPayloadParse(self, hBs, &bitCnt, previous_element, elIndex,
...
bail:
	if((error != AAC_DEC_OK) && fIsFillElement) { /* Skip the remaining extension bytes */
		FDKpushBiDirectional(hBs, *count);
		*count = 0;
...
static AAC_DECODER_ERROR_t aacDecoder_ParseExplicitMpsAndSbr(HANDLE_AACDECODER self, HANDLE_FDK_BITSTREAM bs,
	...
	if((self->flags[0] & AC_SBR_PRESENT) && (self->flags[0] & (AC_USAC | AC_RSVD50 | AC_ELD | AC_DRM))) {
		SBR_ERROR err = SBRDEC_OK;
		int32_t       chElIdx, numChElements = el_cnt[ID_SCE] + el_cnt[ID_CPE] + el_cnt[ID_LFE] + el_cnt[ID_USAC_SCE] +
									 el_cnt[ID_USAC_CPE] + el_cnt[ID_USAC_LFE];
		int32_t bitCntTmp = bitCnt;

		if(self->flags[0] & AC_USAC) { chElIdx = numChElements - 1; }
		...
		for(; chElIdx < numChElements; chElIdx += 1) {
			...
			errTmp = sbrDecoder_Parse(self->hSbrDecoder, bs, self->pDrmBsBuffer, self->drmBsBufferSize, &bitCnt, -1,
									  self->flags[0] & AC_SBRCRC, sbrType, chElIdx, self->flags[0], self->elFlags);
			if(errTmp != SBRDEC_OK) {
				err = errTmp;
				bitCntTmp = bitCnt;
				bitCnt = 0;
	...
	if((bitCnt > 0) && (self->flags[0] & (AC_USAC | AC_RSVD50))) {
		if((self->flags[0] & AC_MPS_PRESENT) || (self->elFlags[element_index] & AC_EL_USAC_MPS212)) {
			int32_t err;

			err = mpegSurroundDecoder_ParseNoHeader((CMpegSurroundDecoder *)self->pMpegSurroundDecoder, bs, &bitCnt,
													self->flags[0] & AC_INDEP);
			if(err != MPS_OK) {
				self->frameOK = 0;
				ErrorStatus = AAC_DEC_PARSE_ERROR;
	...
	if(self->flags[0] & AC_DRM) {
		if((bitCnt = (int32_t)FDKgetValidBits(bs)) != 0) { FDKpushBiDirectional(bs, bitCnt); }
	}
...
static void CAacDecoder_DeInit(HANDLE_AACDECODER self, const int32_t subStreamIndex) {
	int32_t ch;
	int32_t aacChannelOffset = 0, aacChannels = (8);
	int32_t numElements = (3 * ((8) * 2) + (((8) * 2)) / 2 + 4 * (1) + 1), elementOffset = 0;
	...
	for(ch = aacChannelOffset; ch < aacChannelOffset + aacChannels; ch++) {
		if(self->pAacDecoderChannelInfo[ch] != NULL) {
			...
			if(self->pAacDecoderChannelInfo[ch]->pComData != NULL) {
				...
				if(ch < (8) - 1) {
					if((self->pAacDecoderChannelInfo[ch + 1] != NULL) &&
					   (self->pAacDecoderChannelInfo[ch + 1]->pComData == self->pAacDecoderChannelInfo[ch]->pComData)) {
		...
		if(self->pAacDecoderStaticChannelInfo[ch] != NULL) {
			if(self->pAacDecoderStaticChannelInfo[ch]->pOverlapBuffer != NULL) {
				FDKafree(self->pAacDecoderStaticChannelInfo[ch]->pOverlapBuffer);
				self->pAacDecoderStaticChannelInfo[ch]->pOverlapBuffer = NULL;
			}
		...
		if(self->pAacDecoderChannelInfo[ch] != NULL) {
			FDKafree(self->pAacDecoderChannelInfo[ch]);
			self->pAacDecoderChannelInfo[ch] = NULL;
		}
	}
 ...
 * \return error
 */
AAC_DECODER_ERROR_t CAacDecoder_CtrlCFGChange(HANDLE_AACDECODER self, uint8_t flushStatus, int8_t flushCnt,
...
CAacDecoder_Init(HANDLE_AACDECODER self, const CSAudioSpecificConfig_t *asc, uint8_t configMode, uint8_t *configChanged) {
					...
					if(CProgramConfig_IsValid(&asc->m_progrConfigElement)) {
						...
						if(ascChannels > 0) {
							...
							el_tmp = CProgramConfig_GetElementTable(&asc->m_progrConfigElement, self->elements,
																	(((8)) + (8)), &self->chMapIndex);
							for(; el_tmp < (3 * ((8) * 2) + (((8) * 2)) / 2 + 4 * (1) + 1); el_tmp++) {
								self->elements[el_tmp] = ID_NONE;
							}
						}
						else { return AAC_DEC_UNSUPPORTED_CHANNELCONFIG; }
					}
					else {
						self->chMapIndex = 0;
	...
	if(asc->m_aot == AOT_USAC) {
		...
		int32_t numChannel;
		pcmDmx_GetParam(self->hPcmUtils, MIN_NUMBER_OF_OUTPUT_CHANNELS, &numChannel);
		flushChannels = fMin(fMax(numChannel, flushChannels), (8));
	}

	if(IS_USAC(asc->m_aot)) {
		for(int32_t el = 0; el < (int32_t)asc->m_sc.m_usacConfig.m_usacNumElements; el++) {
	...
	elementOffset = 0;
	if((ascChannels <= 0) || (ascChannels > (8)) || (asc->m_channelConfiguration > AACDEC_MAX_CH_CONF)) {
		return AAC_DEC_UNSUPPORTED_CHANNELCONFIG;
	}
	...
	if((asc->m_channelConfiguration > 0) || IS_USAC(asc->m_aot)) {
		if(IS_USAC(asc->m_aot)) {
			...
			if(self->pUsacConfig[streamIndex]->m_usacNumElements > (3 * ((8) * 2) + (((8) * 2)) / 2 + 4 * (1) + 1)) {
				goto bail;
			}

			if(self->numUsacElements[streamIndex] != asc->m_sc.m_usacConfig.m_usacNumElements) { ascChanged = 1; }

			if(configMode & AC_CM_ALLOC_MEM) {
				self->numUsacElements[streamIndex] = asc->m_sc.m_usacConfig.m_usacNumElements;
			}

			self->mpsEnableCurr = 0;
			for(int32_t _el = 0; _el < (int32_t)self->pUsacConfig[streamIndex]->m_usacNumElements; _el++) {
				int32_t el = _el + elementOffset;
				if(self->elements[el] != self->pUsacConfig[streamIndex]->element[_el].usacElementType) {
					ascChanged = 1;
				}
				if(self->usacStereoConfigIndex[el] != asc->m_sc.m_usacConfig.element[_el].m_stereoConfigIndex) {
					ascChanged = 1;
				}
				if(configMode & AC_CM_ALLOC_MEM) {
				...
				self->elFlags[el] |= (asc->m_sc.m_usacConfig.element[_el].m_noiseFilling) ? AC_EL_USAC_NOISE : 0;
				self->elFlags[el] |=
					(asc->m_sc.m_usacConfig.element[_el].m_stereoConfigIndex > 0) ? AC_EL_USAC_MPS212 : 0;
				self->elFlags[el] |= (asc->m_sc.m_usacConfig.element[_el].m_interTes) ? AC_EL_USAC_ITES : 0;
				self->elFlags[el] |= (asc->m_sc.m_usacConfig.element[_el].m_pvc) ? AC_EL_USAC_PVC : 0;
				self->elFlags[el] |=
					(asc->m_sc.m_usacConfig.element[_el].usacElementType == ID_USAC_LFE) ? AC_EL_USAC_LFE : 0;
				self->elFlags[el] |=
			...
			if(self->pUsacConfig[streamIndex]->m_usacNumElements) {
				self->hasAudioPreRoll = asc->m_sc.m_usacConfig.element[0].extElement.usacExtElementHasAudioPreRoll;
			}
			if(configMode & AC_CM_ALLOC_MEM) {
			...
			for(; i < (3 * ((8) * 2) + (((8) * 2)) / 2 + 4 * (1) + 1); i++) { self->elements[i] = ID_NONE; }
		}

		{
			int32_t ch;

			for(ch = 0; ch < ascChannels; ch++) { self->chMapping[ch] = ch; }
			for(; ch < (8); ch++) { self->chMapping[ch] = 255; }
		}
		...
		if(CProgramConfig_IsValid(&asc->m_progrConfigElement)) {
			...
			pcmDmx_SetMatrixMixdownFromPce(self->hPcmUtils, asc->m_progrConfigElement.MatrixMixdownIndexPresent,
	...
	if(self->streamInfo.aot != asc->m_aot) {
		if(configMode & AC_CM_ALLOC_MEM) { self->streamInfo.aot = asc->m_aot; }
		ascChanged = 1;
	}

	if(asc->m_aot == AOT_ER_AAC_ELD && asc->m_sc.m_eldSpecificConfig.m_downscaledSamplingFrequency != 0) {
		if(self->SamplingRateInfo[0].samplingRate != asc->m_sc.m_eldSpecificConfig.m_downscaledSamplingFrequency ||
		   self->SamplingRateInfo[0].samplingRate * self->downscaleFactor != asc->m_samplingFrequency) {
			/* get downscaledSamplingFrequency from ESC and compute the downscale
			 * factor */
			downscaleFactorInBS =
				asc->m_samplingFrequency / asc->m_sc.m_eldSpecificConfig.m_downscaledSamplingFrequency;
			if(downscaleFactorInBS == 1 || downscaleFactorInBS == 2 || downscaleFactorInBS == 3 ||
	...
	if((int32_t)asc->m_samplesPerFrame % downscaleFactor != 0) {
	...
	if(asc->m_aot == AOT_ER_AAC_ELD) {
		if(self->useLdQmfTimeAlign != asc->m_sc.m_eldSpecificConfig.m_useLdQmfTimeAlign) { ascChanged = 1; }
		if(configMode & AC_CM_ALLOC_MEM) {
			self->useLdQmfTimeAlign = asc->m_sc.m_eldSpecificConfig.m_useLdQmfTimeAlign;
		}
		if(self->sbrEnabled != asc->m_sbrPresentFlag) { ascChanged = 1; }
	}

	self->streamInfo.extAot = asc->m_extensionAudioObjectType;
	if(self->streamInfo.extSamplingRate != (int32_t)asc->m_extensionSamplingFrequency) { ascChanged = 1; }
	if(configMode & AC_CM_ALLOC_MEM) { self->streamInfo.extSamplingRate = asc->m_extensionSamplingFrequency; }
	...
	if(self->sbrEnabled && asc->m_extensionSamplingFrequency) {
		...
		if(configMode & AC_CM_ALLOC_MEM) {
			self->streamInfo.extSamplingRate = self->streamInfo.extSamplingRate / self->downscaleFactor;
		}
	}
	...
	if(asc->m_aot == AOT_ER_AAC_ELD) {
		...
		self->flags[streamIndex] |=
			(asc->m_sbrPresentFlag) ? AC_SBR_PRESENT : 0; /* Need to set the SBR flag for backward-compatibility
												reasons. Even if SBR is not supported. */
		self->flags[streamIndex] |= (asc->m_sc.m_eldSpecificConfig.m_sbrCrcFlag) ? AC_SBRCRC : 0;
		self->flags[streamIndex] |= (asc->m_sc.m_eldSpecificConfig.m_useLdQmfTimeAlign) ? AC_MPS_PRESENT : 0;
		if(self->mpsApplicable) { self->mpsEnableCurr = asc->m_sc.m_eldSpecificConfig.m_useLdQmfTimeAlign; }
	}
	...
	if(asc->m_aot == AOT_USAC) {
		...
		self->flags[streamIndex] |= (asc->m_sc.m_usacConfig.element[0].m_stereoConfigIndex > 0) ? AC_MPS_PRESENT : 0;
	}
	...
	if((self->SamplingRateInfo[streamIndex].samplingRate != asc->m_samplingFrequency) ||
	   (self->streamInfo.aacSamplesPerFrame != (int32_t)asc->m_samplesPerFrame / downscaleFactor)) {
		AAC_DECODER_ERROR_t error;
		...
		if(configMode & AC_CM_ALLOC_MEM) {
			...
			if(error != AAC_DEC_OK) { return error; }
			self->streamInfo.aacSampleRate = self->SamplingRateInfo[0].samplingRate / self->downscaleFactor;
			self->streamInfo.aacSamplesPerFrame = asc->m_samplesPerFrame / self->downscaleFactor;
			if(self->streamInfo.aacSampleRate <= 0) { return AAC_DEC_UNSUPPORTED_SAMPLINGRATE; }
		}
	}

	/* Check if amount of channels has changed. */
	if(self->ascChannels[streamIndex] != ascChannels) { ascChanged = 1; }

	/* detect config change */
	if(configMode & AC_CM_DET_CFG_CHANGE) {
		if(ascChanged != 0) { *configChanged = 1; }
		return err;
	}
	...
	switch(asc->m_aot) {
		case AOT_USAC:
			if(self->sbrEnabled) {
				for(int32_t _el = 0; _el < (int32_t)self->pUsacConfig[streamIndex]->m_usacNumElements; _el++) {
					int32_t el = elementOffset + _el;
					if(IS_USAC_CHANNEL_ELEMENT(self->elements[el])) {
						if(usacStereoConfigIndex < 0) { usacStereoConfigIndex = self->usacStereoConfigIndex[el]; }
						else {
				...
				if(usacStereoConfigIndex < 0) { goto bail; }

				if(usacStereoConfigIndex == 3) { self->flags[streamIndex] |= AC_USAC_SCFGI3; }
			}
			break;
		default:
			break;
	...
	if(*configChanged) {
		...
		*/
			...
			case AOT_USAC:
				if(self->sbrEnabled) {
					...
					self->qmfDomain.globalConf.nBandsAnalysis_requested =
						map_sbrRatio_2_nAnaBands[asc->m_sc.m_usacConfig.m_sbrRatioIndex - 1];
					...
					self->qmfDomain.globalConf.parkChannel = (usacStereoConfigIndex == 3) ? 1 : 0;
					self->qmfDomain.globalConf.parkChannel_requested = (usacStereoConfigIndex == 3) ? 1 : 0;
					self->qmfDomain.globalConf.qmfDomainExplicitConfig = 1;
				}
				break;
			case AOT_ER_AAC_ELD:
				if(self->mpsEnableCurr && asc->m_sc.m_eldSpecificConfig.m_useLdQmfTimeAlign) {
				...
				break;
			default:
				self->qmfDomain.globalConf.qmfDomainExplicitConfig = 0; /* qmfDomain is initialized by SBR and MPS init
		...
		{
			int32_t ch = aacChannelsOffset;
			for(int32_t _ch = 0; _ch < ascChannels; _ch++) {
				if(ch >= (8)) { goto bail; }
				self->pAacDecoderChannelInfo[ch] =
                                (CAacDecoderChannelInfo_t *) FDKaalloc((1) * sizeof(CAacDecoderChannelInfo_t), 8);
			...
			int32_t chIdx = aacChannelsOffsetIdx;
			ch = aacChannelsOffset;
			int32_t _numElements;
			_numElements = (((8)) + (8));
			if(self->flags[streamIndex] & (AC_RSV603DA | AC_USAC)) {
				_numElements = (int32_t)asc->m_sc.m_usacConfig.m_usacNumElements;
			}
			for(int32_t _el = 0; _el < _numElements; _el++) {
				int32_t el_channels = 0;
				int32_t el = elementOffset + _el;

				if(self->flags[streamIndex] & (AC_ER | AC_LD | AC_ELD | AC_RSV603DA | AC_USAC | AC_RSVD50)) {
					if(ch >= ascChannels) { break; }
				}
				...
				switch(self->elements[el]) {
					...
					case ID_USAC_LFE:
						...
						el_channels = CAacDecoder_GetELChannels(self->elements[el], self->usacStereoConfigIndex[el]);

						{
							...
							self->pAacDecoderChannelInfo[ch]->pSpectralCoefficient =
								(int32_t*)&self->workBufferCore2[ch * 1024];

							if(el_channels == 2) {
								if(ch >= (8) - 1) { return AAC_DEC_UNSUPPORTED_CHANNELCONFIG; }
								...
								self->pAacDecoderChannelInfo[ch + 1]->pSpectralCoefficient =
									(int32_t*)&self->workBufferCore2[(ch + 1) * 1024];
							}
				...
				if(self->elements[el] == ID_END) { break; }
			...
			for(int32_t _ch = 0; _ch < ascChannels; _ch++) {
				...
				{
                    CAacDecoderStaticChannelInfo_t *chi = NULL;
                    chi = (CAacDecoderStaticChannelInfo_t *) FDKcalloc(1, sizeof(CAacDecoderStaticChannelInfo_t));
                    self->pAacDecoderStaticChannelInfo[ch] = chi;
					...
					if(self->flags[streamIndex] & (AC_USAC | AC_RSVD50 | AC_RSV603DA /*|AC_BSAC*/)) {
						self->pAacDecoderStaticChannelInfo[ch]->hArCo = CArco_Create();
						if(self->pAacDecoderStaticChannelInfo[ch]->hArCo == NULL) { goto bail; }
					}

					if(!(self->flags[streamIndex] & (AC_USAC | AC_RSV603DA))) {
						CPns_UpdateNoiseState(&self->pAacDecoderChannelInfo[ch]->data.aac.PnsData,
			...
			if(self->flags[streamIndex] & (AC_USAC | AC_RSV603DA)) {
				int32_t complexStereoPredPossible = 0;
				ch = aacChannelsOffset;
				chIdx = aacChannelsOffsetIdx;
				for(int32_t _el2 = 0; _el2 < (int32_t)asc->m_sc.m_usacConfig.m_usacNumElements; _el2++) {
					int32_t el2 = elementOffset + _el2;
					int32_t elCh = 0, ch2;

					if((self->elements[el2] == ID_USAC_CPE) && !(self->usacStereoConfigIndex[el2] == 1)) { elCh = 2; }
					else if(IS_CHANNEL_ELEMENT(self->elements[el2])) { elCh = 1; }

					if(self->elFlags[el2] & AC_EL_USAC_CP_POSSIBLE) {
						complexStereoPredPossible = 1;
						if(self->cpeStaticData[el2] == NULL) {
							self->cpeStaticData[el2] = (CpePersistentData_t *)FDKcalloc(1, sizeof(CpePersistentData_t));
					...
					if(complexStereoPredPossible && (elCh == 2)) {
						...
						if(self->pAacDecoderChannelInfo[ch - 1]->pComStaticData->cplxPredictionData == NULL) {
							self->pAacDecoderChannelInfo[ch - 1]->pComStaticData->cplxPredictionData =
								(CCplxPredictionData_t *) FDKcalloc(1, sizeof(CCplxPredictionData_t));
						}
						if(self->pAacDecoderChannelInfo[ch - 1]->pComStaticData->cplxPredictionData == NULL) {
							goto bail;
						}
	...
	if(self->pAacDecoderChannelInfo[0] != NULL) {
		self->pDrmBsBuffer = self->pAacDecoderChannelInfo[0]->pComStaticData->pWorkBufferCore1->DrmBsBuffer;
	...
	if(*configChanged) {
		...
		int32_t ch = 0;
		int32_t chIdx = 0;
		for(int32_t _ch = 0; _ch < self->ascChannels[streamIndex]; _ch++) {
			switch(self->streamInfo.aot) {
				case AOT_ER_AAC_ELD:
				case AOT_ER_AAC_LD:
			...
			CConcealment_InitChannelData(&self->pAacDecoderStaticChannelInfo[ch]->concealmentInfo,
										 &self->concealCommonData, initRenderMode, self->streamInfo.aacSamplesPerFrame);
			ch++;
			chIdx++;
		}
	}
	...
	if(*configChanged) {
		int32_t drcDecSampleRate, drcDecFrameSize;
		...
		if(FDK_drcDec_Init(self->hUniDrcDecoder, drcDecFrameSize, drcDecSampleRate, self->aacChannels) != 0) goto bail;
	}
...
AAC_DECODER_ERROR_t CAacDecoder_DecodeFrame(HANDLE_AACDECODER self, const uint32_t flags, int32_t *pTimeData,
	...
	int32_t checkSampleRate = self->streamInfo.aacSampleRate;

	int32_t CConceal_TDFading_Applied[(8)] = {0}; /* Initialize status of Time Domain fading */

	if(self->aacChannels <= 0) { return AAC_DEC_UNSUPPORTED_CHANNELCONFIG; }
	...
	if(!(flags & (AACDEC_CONCEAL | AACDEC_FLUSH)) && !(self->flags[0] & (AC_USAC | AC_RSV603DA))) {
		int32_t ch;
		if(self->streamInfo.channelConfig == 0) {
			/* Init Channel/Element mapping table */
			for(ch = 0; ch < (8); ch++) { self->chMapping[ch] = 255; }
			if(!CProgramConfig_IsValid(pce)) {
				int32_t el;
				for(el = 0; el < (3 * ((8) * 2) + (((8) * 2)) / 2 + 4 * (1) + 1); el++) {
	...
	if(self->downscaleFactor > 1 && (self->flags[0] & AC_ELD)) { self->flags[0] |= AC_ELD_DOWNSCALE; }
	else { self->flags[0] &= ~AC_ELD_DOWNSCALE; }
	/* unsupported dsf (aacSampleRate has not yet been divided by dsf) -> divide
	 */
	if(self->downscaleFactorInBS > 1 && (self->flags[0] & AC_ELD_DOWNSCALE) == 0) {
		checkSampleRate = self->streamInfo.aacSampleRate / self->downscaleFactorInBS;
	}
	...
	switch(checkSampleRate) {
		...
		case 22050:
			break;
		default:
			if(!(self->flags[0] & (AC_USAC | AC_RSVD50 | AC_RSV603DA))) { return AAC_DEC_UNSUPPORTED_SAMPLINGRATE; }
			break;
	}
	...
	while((type != ID_END) && (!(flags & (AACDEC_CONCEAL | AACDEC_FLUSH))) && self->frameOK) {
		...
		if((self->flags[streamIndex] & (AC_USAC | AC_RSVD50) && element_count == 0) ||
		   (self->flags[streamIndex] & AC_RSV603DA)) {
			...
			if(FDKreadBit(bs)) { self->flags[streamIndex] |= AC_INDEP; }

			int32_t ch = aacChannels;
			for(int32_t chIdx = aacChannels; chIdx < self->ascChannels[streamIndex]; chIdx++) {
				{
					/* Robustness check */
					if(ch >= self->aacChannels) { return AAC_DEC_UNKNOWN; }
					...
					if((CConcealment_GetLastFrameOk(&self->pAacDecoderStaticChannelInfo[ch]->concealmentInfo, 1) ==
						0) &&
					   !(self->flags[streamIndex] & AC_INDEP)) {
						self->frameOK = 0;
					}
		...
		if((int32_t)FDKgetValidBits(bs) < 0) { self->frameOK = 0; }
		...
		switch(type) {
			...
			case ID_USAC_LFE:
				 ...
				 */
				{
					int32_t totalAscChannels = 0;

					for(int32_t i = 0; i < (1 * 1); i++) { totalAscChannels += self->ascChannels[i]; }
				...
				if(self->frameOK) {
					ErrorStatus = CChannelElement_Read(
						...
						&self->SamplingRateInfo[streamIndex], self->flags[streamIndex], self->elFlags[element_count],
						self->streamInfo.aacSamplesPerFrame, el_channels, self->streamInfo.epConfig, self->hInput);
				...
				if(self->frameOK) {
					 ...
					 * program */
					if(CProgramConfig_LookupElement(pce, self->streamInfo.channelConfig,
													self->pAacDecoderChannelInfo[aacChannels]->ElementInstanceTag,
													aacChannels, self->chMapping, self->channelType,
													self->channelIndices, (8), &previous_element_index, self->elements,
													type)) {
					...
					if(self->frameOK && ((self->flags[streamIndex] & AC_SBR_PRESENT) || (self->sbrEnabled == 1)) &&
					   !(self->flags[streamIndex] & AC_USAC) /* Is done during explicit config set up */
						...
						sbrError = sbrDecoder_InitElement(self->hSbrDecoder, self->streamInfo.aacSampleRate,
														  ...
														  type, previous_element_index, 2, /* Signalize that harmonicSBR
				...
				if(self->frameOK && (self->flags[streamIndex] & AC_USAC) &&
				   (type == ID_USAC_CPE || type == ID_USAC_SCE)) {
					ErrorStatus = aacDecoder_ParseExplicitMpsAndSbr(self, bs, previous_element, previous_element_index,
			...
			case ID_CCE:
				...
				*/
				...
				if(self->frameOK) {
					...
					CAacDecoderChannelInfo_t tmpAacDecoderChannelInfo, *pTmpAacDecoderChannelInfo;
					...
					tmpAacDecoderChannelInfo.pSpectralCoefficient = (int32_t*)mdctSpec;
					/* Assume AAC-LC */
					tmpAacDecoderChannelInfo.granuleLength = self->streamInfo.aacSamplesPerFrame / 8;
					/* Reset PNS data. */
					CPns_ResetData(&tmpAacDecoderChannelInfo.data.aac.PnsData,
								   &tmpAacDecoderChannelInfo.pComData->pnsInterChannelData);
					pTmpAacDecoderChannelInfo = &tmpAacDecoderChannelInfo;
					/* do CCE parsing */
					ErrorStatus = CChannelElement_Read(bs, &pTmpAacDecoderChannelInfo, NULL, self->streamInfo.aot,
			...
			case ID_DSE: {
				...
				if(!CProgramConfig_LookupElement(pce, self->streamInfo.channelConfig, element_instance_tag, 0,
												 self->chMapping, self->channelType, self->channelIndices, (8),
												 &previous_element_index, self->elements, type)) {
			...
			case ID_PCE: {
				int32_t result =
					CProgramConfigElement_Read(bs, self->hInput, pce, self->streamInfo.channelConfig, auStartAnchor);
				if(result < 0) {
					...
					/* Built element table */
					int32_t elIdx = CProgramConfig_GetElementTable(pce, self->elements, (((8)) + (8)), &self->chMapIndex);
					/* Reset the remaining tabs */
					for(; elIdx < (3 * ((8) * 2) + (((8) * 2)) / 2 + 4 * (1) + 1); elIdx++) {
					 ...
					 * inconsistencies */
					if(element_count != 0) { self->frameOK = 0; }
				}
				pceRead = (result >= 0) ? 1 : 0;
			} break;

			case ID_FIL: {
				int32_t bitCnt = FDKreadBits(bs, 4); /* bs_count */

				if(bitCnt == 15) {
					int32_t esc_count = FDKreadBits(bs, 8); /* bs_esc_count */
					bitCnt = esc_count + 14;
				}
				...
				while(bitCnt > 0) {
					ErrorStatus =
						CAacDecoder_ExtPayloadParse(self, bs, &bitCnt, previous_element, previous_element_index, 1);
					if(ErrorStatus != AAC_DEC_OK) {
						self->frameOK = 0;
						break;
					}
			...
			case ID_USAC_EXT: {
				if((element_count - element_count_prev_streams) >= TP_USAC_MAX_ELEMENTS) {
					self->frameOK = 0;
					ErrorStatus = AAC_DEC_PARSE_ERROR;
					break;
				}
				 ...
				 */
				int32_t usacExtElementPayloadLength;
				/* int32_t usacExtElementStart, usacExtElementStop; */
				...
				if(FDKreadBit(bs)) {     /* usacExtElementPresent */
					if(FDKreadBit(bs)) { /* usacExtElementUseDefaultLength */
						...
						if(usacExtElementPayloadLength == (uint32_t)(1 << 8) - 1) {
							uint32_t valueAdd = FDKreadBits(bs, 16);
							usacExtElementPayloadLength += (int32_t)valueAdd - 2;
						}
					}
					if(usacExtElementPayloadLength > 0) {
						int32_t usacExtBitPos;
						...
						switch(usacExtElementType) {
							case ID_EXT_ELE_UNI_DRC: /* uniDrcGain() */
								if(streamIndex == 0) {
									...
									if(drcErr != 0) { ErrorStatus = AAC_DEC_PARSE_ERROR; }
								}
								break;
						...
						usacExtBitPos = (usacExtElementPayloadLength * 8) - (usacExtBitPos - (int32_t)FDKgetValidBits(bs));
						if(usacExtBitPos < 0) {
							self->frameOK = 0;
							ErrorStatus = AAC_DEC_PARSE_ERROR;
						}
	...
	if(!(flags & (AACDEC_CONCEAL | AACDEC_FLUSH))) {
		...
		if(transportDec_GetAuBitsTotal(self->hInput, 0) > 0) {
			...
			if(self->prerollAULength[self->accessUnit]) {
				unreadBits = unreadBits - transportDec_GetAuBitsTotal(self->hInput, 0) +
							 (int32_t)self->prerollAULength[self->accessUnit];
			}
			if(((self->flags[streamIndex] & (AC_RSVD50 | AC_USAC)) && ((unreadBits < 0) || (unreadBits > 7)) &&
			   ...
			   ((!(self->flags[streamIndex] & (AC_RSVD50 | AC_USAC)) || (self->prerollAULength[self->accessUnit])) &&
				...
				if((((unreadBits < 0) || (unreadBits > 7)) && self->frameOK) &&
				   ((transportDec_GetFormat(self->hInput) == TT_DRM) && (self->flags[streamIndex] & AC_USAC))) {
				...
				if(ErrorStatus == AAC_DEC_OK && self->frameOK == 0) { ErrorStatus = AAC_DEC_PARSE_ERROR; }
		...
		if(self->frameOK && type != ID_END) {
			/* Do not overwrite current error */
			if(ErrorStatus == AAC_DEC_OK) { ErrorStatus = AAC_DEC_PARSE_ERROR; }
			self->frameOK = 0;
		}
	}
	...
	while(type != ID_END && element_count < (3 * ((8) * 2) + (((8) * 2)) / 2 + 4 * (1) + 1)) {
		int32_t el_channels;

		if((flags & (AACDEC_CONCEAL | AACDEC_FLUSH)) || !self->frameOK) {
			channel_elements[element_count] = self->elements[element_count];
			if(channel_elements[element_count] == ID_NONE) { channel_elements[element_count] = ID_END; }
		}

		if(self->flags[streamIndex] & (AC_USAC | AC_RSV603DA | AC_BSAC)) { type = self->elements[element_count]; }
		else { type = channel_elements[element_count]; }
	...
	if((aacChannels == 0 || aacChannels > self->aacChannels) && !(flags & (AACDEC_CONCEAL | AACDEC_FLUSH))) {
		/* Do not overwrite current error */
		if(ErrorStatus == AAC_DEC_OK) { ErrorStatus = AAC_DEC_DECODE_FRAME_ERROR; }
		self->frameOK = 0;
		aacChannels = 0;
	}
	...
	if((self->frameOK == 0) && (ErrorStatus == AAC_DEC_OK)) { ErrorStatus = AAC_DEC_DECODE_FRAME_ERROR; }
	...
	if(aacChannels == 0) { ErrorStatus = AAC_DEC_UNKNOWN; }

	if(pceRead == 1 && CProgramConfig_IsValid(pce)) {
		/* Set matrix mixdown infos if available from PCE. */
		pcmDmx_SetMatrixMixdownFromPce(self->hPcmUtils, pce->MatrixMixdownIndexPresent, pce->MatrixMixdownIndex,
									   pce->PseudoSurroundEnable);
		;
	}

	/* If there is no valid data to transfrom into time domain, return. */
	if(!IS_OUTPUT_VALID(ErrorStatus)) { return ErrorStatus; }
	...
	*/
	{
		...
		aacDecoder_drcSetParam(self->hDrcInfo, UNIDRC_PRECEDENCE,
		 ...
		 */
		mapped = aacDecoder_drcProlog(self->hDrcInfo, bs, self->pAacDecoderStaticChannelInfo, pce->ElementInstanceTag,
									  drcChMap, aacChannels);
		if(mapped > 0) {
			if(!(self->flags[streamIndex] & (AC_USAC | AC_RSV603DA))) {
		...
		for(el = 0; el < element_count; el++) {
			int32_t frameOk_butConceal = 0; /* Force frame concealment during mute release active state. */
			int32_t concealApplyReturnCode;

			if(self->flags[streamIndex] & (AC_USAC | AC_RSV603DA | AC_BSAC)) { type = self->elements[el]; }
			else { type = channel_elements[el]; }

			{
				int32_t nElementChannels;

				nElementChannels = CAacDecoder_GetELChannels(type, self->usacStereoConfigIndex[el]);

				el_channels += nElementChannels;

				if(nElementChannels == 0) { continue; }
			}
			...
			for(; cIdx < el_channels; c++, cIdx++, elCh++) {
				/* Robustness check */
				if(c >= aacChannels) { return AAC_DEC_UNKNOWN; }

				CAacDecoderChannelInfo_t       *pAacDecoderChannelInfo = self->pAacDecoderChannelInfo[c];
				CAacDecoderStaticChannelInfo_t *pAacDecoderStaticChannelInfo = self->pAacDecoderStaticChannelInfo[c];

				/* Setup offset for time buffer traversal. */
				{
					pAacDecoderStaticChannelInfo = self->pAacDecoderStaticChannelInfo[Reverse_chMapping[c]];
					offset = FDK_chMapDescr_getMapValue(&self->mapDescr, Reverse_chMapping[cIdx], self->chMapIndex) *
				 ...
				 * following concealment method, mark the frame as erroneous */
				{
					...
					const int32_t icsIsInvalid =
						(GetScaleFactorBandsTransmitted(pIcsInfo) > GetScaleFactorBandsTotal(pIcsInfo));
					const int32_t icsInfoUsedinFadeOut = !(pAacDecoderChannelInfo->renderMode == AACDEC_RENDER_LPD &&
													   pAacDecoderStaticChannelInfo->last_lpd_mode == 0);
					if(icsInfoUsedinFadeOut && icsIsInvalid && !mute_release_active) { self->frameOK = 0; }
				}
				...
				*/
				{
					CAacDecoderChannelInfo_t       **ppAacDecoderChannelInfo = &pAacDecoderChannelInfo;
					CAacDecoderStaticChannelInfo_t **ppAacDecoderStaticChannelInfo = &pAacDecoderStaticChannelInfo;
					{
						concealApplyReturnCode = CConcealment_Apply(
							...
							*ppAacDecoderStaticChannelInfo, &self->SamplingRateInfo[streamIndex],
							self->streamInfo.aacSamplesPerFrame, pAacDecoderStaticChannelInfo->last_lpd_mode,
							(self->frameOK && !(flags & AACDEC_CONCEAL)), self->flags[streamIndex]);
					}
				}
				if(concealApplyReturnCode == -1) { frameOk_butConceal = 1; }
				...
				if(self->flushStatus && (self->flushCnt > 0) && !(flags & AACDEC_CONCEAL)) {
					...
					switch(pAacDecoderChannelInfo->renderMode) {
						...
						case AACDEC_RENDER_ELDFB: {
							CBlock_FrequencyToTimeLowDelay(pAacDecoderStaticChannelInfo, pAacDecoderChannelInfo,
														   pTimeData + offset, self->streamInfo.aacSamplesPerFrame);
							self->extGainDelay = (self->streamInfo.aacSamplesPerFrame * 2 -
		...
		if(self->flags[streamIndex] & AC_USAC) {
			int32_t bsPseudoLr = 0;
			mpegSurroundDecoder_IsPseudoLR((CMpegSurroundDecoder *)self->pMpegSurroundDecoder, &bsPseudoLr);
			/* ISO/IEC 23003-3, 7.11.2.6 Modification of core decoder output (pseudo
			 * LR) */
			if((aacChannels == 2) && bsPseudoLr) {
				int32_t            i, offset2;
				const int16_t invSqrt2 = FL2FXCONST_SGL(0.707106781186547f);
		...
		mapped = aacDecoder_drcEpilog(self->hDrcInfo, bs, self->pAacDecoderStaticChannelInfo, pce->ElementInstanceTag,
									  drcChMap, aacChannels);
		if(mapped > 0) {
			if(!(self->flags[streamIndex] & (AC_USAC | AC_RSV603DA))) {
				/* If at least one DRC thread has been mapped to a channel there was DRC
	...
	aacDecoder_drcGetInfo(self->hDrcInfo, &self->streamInfo.drcPresMode, &self->streamInfo.drcProgRefLev);
