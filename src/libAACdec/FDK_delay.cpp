...
int32_t FDK_Delay_Create(FDK_SignalDelay_t* data, const uint16_t delay, const uint8_t num_channels) {
    ...
    if(delay > 0) {
        data->delay_line = (int32_t*)FDKcalloc(num_channels * delay, sizeof(int32_t));
        if(data->delay_line == NULL) { return -1; }
    }
    else { data->delay_line = NULL; }
...
void FDK_Delay_Apply(FDK_SignalDelay_t* data, int32_t* time_buffer, const uint32_t frame_length, const uint8_t channel) {
    ...
    if(data->delay > 0) {
        ...
        if(frame_length >= data->delay) {
            memcpy(tmp, &time_buffer[frame_length - data->delay], data->delay * sizeof(int32_t));
            memmove(&time_buffer[data->delay], &time_buffer[0], (frame_length - data->delay) * sizeof(int32_t));

            memcpy(&time_buffer[0], &data->delay_line[channel * data->delay], data->delay * sizeof(int32_t));
            memcpy(&data->delay_line[channel * data->delay], tmp, data->delay * sizeof(int32_t));
        }
        else {
            memcpy(tmp, &time_buffer[0], frame_length * sizeof(int32_t));
            memcpy(&time_buffer[0], &data->delay_line[channel * data->delay], frame_length * sizeof(int32_t));
            memcpy(&data->delay_line[channel * data->delay], &data->delay_line[channel * data->delay + frame_length],
                   (data->delay - frame_length) * sizeof(int32_t));
            memcpy(&data->delay_line[channel * data->delay + (data->delay - frame_length)], tmp,
