...
typedef enum {
    ...
    AAC_METADATA_PROFILE = 0x0020,             /*!< See ::AAC_MD_PROFILE_t for all available values. */
    ...
    AAC_CONCEAL_METHOD = 0x0100,               /*!< Error concealment: Processing method. 0: Spectral muting. 1: Noise substitution
        ...
        supported). */
    AAC_DRC_BOOST_FACTOR = 0x0200,             /*!< MPEG-4 / MPEG-D Dynamic Range Control (DRC): Scaling factor for boosting gain values. Defines how
        the boosting DRC factors (conveyed in the bitstream) will be applied to the decoded signal. The valid values range from 0 (don't apply boost
        factors) to 127 (fully apply boost factors). Default value is 0 for MPEG-4 DRC and 127 for MPEG-D DRC. */
    AAC_DRC_ATTENUATION_FACTOR = 0x0201,        /*!< MPEG-4 / MPEG-D DRC: Scaling factor  for attenuating gain values.
        Same as ::AAC_DRC_BOOST_FACTOR but for attenuating DRC factors. */
    AAC_DRC_REFERENCE_LEVEL = 0x0202,           /*!< MPEG-4 / MPEG-D DRC: Target reference level / decoder target loudness. Defines the level below
        full-scale (quantized in steps of 0.25dB) to which the output audio signal will be normalized to by the DRC module.\n The parameter controls
        loudness normalization for both MPEG-4 DRC and MPEG-D DRC. The valid values range from 40 (-10 dBFS) to 127 (-31.75 dBFS). Example values:
        124 (-31 dBFS) for audio/video receivers (AVR) or other devices allowing audio playback with high dynamic range, 96(-24 dBFS) for TV sets or
        equivalent devices (default), 64 (-16 dBFS) for mobile devices where the dynamic range of audio playback is restricted.n Any value smaller
        than 0 switches off loudness normalization and MPEG-4 DRC. */
    AAC_DRC_HEAVY_COMPRESSION = 0x0203,         /*!< MPEG-4 DRC: En-/Disable DVB specific heavy compression (aka RF mode). If set to 1, the decoder
        will apply the compression values from the DVB specific ancillary data field. At the same time the MPEG-4 Dynamic Range Control tool will be
        disabled. By default, heavy compression is disabled. */
    AAC_DRC_DEFAULT_PRESENTATION_MODE = 0x0204, /*!< MPEG-4 DRC: Default presentation mode (DRC parameter handling). Defines the handling of the
        DRC parameters boost factor, attenuation factor and heavy compression, if no presentation mode is indicated in the bitstream. For options,
        see ::AAC_DRC_DEFAULT_PRESENTATION_MODE_OPTIONS. Default: ::AAC_DRC_PARAMETER_HANDLING_DISABLED */
    AAC_DRC_ENC_TARGET_LEVEL = 0x0205,          /*!< MPEG-4 DRC: Encoder target level for light (i.e. not heavy) compression. If known, this
        declares the target reference level that was assumed at the encoder for calculation of limiting gains. The valid values range from 0
        (full-scale) to 127 (31.75 dB below full-scale). This parameter is used only with ::AAC_DRC_PARAMETER_HANDLING_ENABLED and ignored
        otherwise. Default: 127 (worst-case assumption). */
    AAC_UNIDRC_SET_EFFECT = 0x0206,             /*!< MPEG-D DRC: Request a DRC effect type for selection of a DRC set. Supported indices are:
        -1: DRC off. Completely disables MPEG-D DRC. 0: None (default). Disables MPEG-D DRC, but automatically enables DRC if necessary to prevent
        clipping. 1: Late night 2: Noisy environment 3: Limited playback range 4: Low playback level 5: Dialog enhancement 6: General
        compression. Used for generally enabling MPEG-D DRC without particular request.*/
    AAC_UNIDRC_ALBUM_MODE = 0x0207,             /*!<  MPEG-D DRC: Enable album mode. 0: Disabled (default), 1: Enabled. Disabled album mode leads to
        application of gain sequences for fading in and out, if provided in the bitstream. Enabled album mode makes use of dedicated album
        loudness information, if provided in the bitstream. */
    AAC_QMF_LOWPOWER = 0x0300,                  /*!< Quadrature Mirror Filter (QMF) Bank processing mode. -1: Use internal default. 0: Use complex
        QMF data mode.  1: Use real (low power) QMF data mode.  */
    AAC_TPDEC_CLEAR_BUFFER = 0x0603             /*!< Clear internal bit stream buffer of transport layers. The decoder will start decoding at new
        data passed after this event and any previous data is discarded. */
} AACDEC_PARAM_t;
typedef struct {
    ...
    int32_t frameSize;      /*!< The frame size of the decoded PCM audio signal. Typically this is: 1024 or 960 for AAC-LC
                             2048 or 1920 for HE-AAC (v2) 512 or 480 for AAC-LD and AAC-ELD  768, 1024, 2048 or 4096 for USAC  */
    int32_t numChannels;    /*!< The number of output audio channels before the rendering module, i.e. the original channel configuration. */
    AUDIO_CHANNEL_TYPE_t *pChannelType;    /*!< Audio channel type of each output audio channel. */
    uint8_t              *pChannelIndices; /*!< Audio channel index for each output audio  channel. See ISO/IEC 13818-7:2005(E), 8.5.3.2
                                            Explicit channel mapping using a program_config_element() */
    int32_t aacSampleRate;  /*!< Sampling rate in Hz without SBR (from configuration  info) divided by a (ELD) downscale factor if present. */
    int32_t profile;        /*!< MPEG-2 profile (from file header) (-1: not applicable (e. g. MPEG-4)).  */
    AUDIO_OBJECT_TYPE_t aot;    /*!< Audio Object Type (from ASC): is set to the appropriate value for MPEG-2 bitstreams (e. g. 2 for AAC-LC). */
    int32_t channelConfig;      /*!< Channel configuration (0: PCE defined, 1: mono, 2: stereo, ...                       */
    int32_t bitRate;            /*!< Instantaneous bit rate.                   */
    int32_t aacSamplesPerFrame; /*!< Samples per frame for the AAC core (from ASC)  divided by a (ELD) downscale factor if present. Typically this is
                                  (with a downscale factor of 1): 1024 or 960 for AAC-LC \n   512 or 480 for AAC-LD   and AAC-ELD  */
    int32_t aacNumChannels;     /*!< The number of audio channels after AAC core processing (before PS or MPS processing).  CAUTION: This
                                 are not the final number of output channels! */
    AUDIO_OBJECT_TYPE_t extAot;          /*!< Extension Audio Object Type (from ASC)   */
    int32_t             extSamplingRate; /*!< Extension sampling rate in Hz (from ASC) divided by a (ELD) downscale factor if present. */
    ...
    int32_t outputLoudness; /*!< Audio output loudness in steps of -0.25 dB. Range: 0 (0 dBFS) to 231 (-57.75 dBFS). A value of -1
        indicates that no loudness metadata is present.  If loudness normalization is active, the value corresponds to the target loudness value set
        with ::AAC_DRC_REFERENCE_LEVEL.\n  If loudness normalization is not active, the output loudness value corresponds to the loudness metadata
