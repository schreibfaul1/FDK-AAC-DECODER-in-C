...
AAC_DECODER_ERROR_t IcsReadMaxSfb(HANDLE_FDK_BITSTREAM bs, CIcsInfo_t *pIcsInfo, const SamplingRateInfo_t *pSamplingRateInfo) {
    ...
    if(pIcsInfo->MaxSfBands > pIcsInfo->TotalSfBands) { ErrorStatus = AAC_DEC_PARSE_ERROR; }

    return ErrorStatus;
}

AAC_DECODER_ERROR_t IcsRead(HANDLE_FDK_BITSTREAM bs, CIcsInfo_t *pIcsInfo, const SamplingRateInfo_t *pSamplingRateInfo, const uint32_t flags) {
    AAC_DECODER_ERROR_t ErrorStatus = AAC_DEC_OK;
    ...
    if(flags & AC_ELD) {
        ...
        if(!(flags & (AC_USAC | AC_RSVD50 | AC_RSV603DA))) { FDKreadBits(bs, 1); }
        pIcsInfo->WindowSequence = (BLOCK_TYPE_t)FDKreadBits(bs, 2);
        pIcsInfo->WindowShape = (uint8_t)FDKreadBits(bs, 1);
        if(flags & AC_LD) {
            if(pIcsInfo->WindowShape) { pIcsInfo->WindowShape = 2; /* select low overlap instead of KBD */ }
        }
    }
    ...
    if(ErrorStatus != AAC_DEC_OK) { goto bail; }

    if(IsLongBlock(pIcsInfo)) {
        if(!(flags & (AC_ELD | AC_SCALABLE | AC_BSAC | AC_USAC | AC_RSVD50 | AC_RSV603DA))) /* If not ELD nor Scalable nor BSAC nor USAC
                                                                                               syntax then ... */
        {
        ...
        for(i = 0; i < (8 - 1); i++) {
            ...
            if(pIcsInfo->ScaleFactorGrouping & mask) { pIcsInfo->WindowGroupLength[pIcsInfo->WindowGroups]++; }
            else { pIcsInfo->WindowGroups++; }
        }
...
AAC_DECODER_ERROR_t getSamplingRateInfo(SamplingRateInfo_t *t, uint32_t samplesPerFrame, uint32_t samplingRateIndex, uint32_t samplingRate) {
    ...
    if((samplingRateIndex >= 15) || (samplesPerFrame == 768)) {
        const uint32_t borders[] = {(uint32_t)-1, 92017, 75132, 55426, 46009, 37566, 27713, 23004, 18783, 13856, 11502, 9391};
        uint32_t       i, samplingRateSearch = samplingRate;

        if(samplesPerFrame == 768) { samplingRateSearch = (samplingRate * 4) / 3; }

        for(i = 0; i < 11; i++) {
            if(borders[i] > samplingRateSearch && samplingRateSearch >= borders[i + 1]) { break; }
        }
        samplingRateIndex = i;
    }
    ...
    assert((uint32_t)t->ScaleFactorBands_Long[t->NumberOfScaleFactorBands_Long] == samplesPerFrame);
    assert(t->ScaleFactorBands_Short == NULL || (uint32_t)t->ScaleFactorBands_Short[t->NumberOfScaleFactorBands_Short] * 8 == samplesPerFrame);
