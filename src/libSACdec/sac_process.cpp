 ...
 */
static inline int32_t interpolateParameter(const int16_t alpha, const int32_t a, const int32_t b) {
...
int32_t getChGain(spatialDec *self, uint32_t ch, int32_t *scale) {
    ...
    *scale = 0;
    ...
    if((!isTwoChMode(self->upmixType)) && (self->upmixType != UPMIXTYPE_BYPASS)) {
        if((ch == 0) || (ch == 1) || (ch == 2)) { /* no modifier */ }
    }

    return gain;
}

SACDEC_ERROR SpatialDecQMFAnalysis(spatialDec *self, const PCM_MPS *inData, const int32_t ts, const int32_t bypassMode,
                                   int32_t **qmfReal, int32_t **qmfImag, const int32_t numInputChannels) {
    SACDEC_ERROR err = MPS_OK;
    int32_t      ch, offset;

    offset = self->pQmfDomain->globalConf.nBandsSynthesis * self->pQmfDomain->globalConf.nQmfTimeSlots;

    {
        for(ch = 0; ch < numInputChannels; ch++) {
            const PCM_MPS *inSamples = &inData[ts * self->pQmfDomain->globalConf.nBandsAnalysis];
            int32_t       *pQmfRealAnalysis = qmfReal[ch]; /* no delay in blind mode */
            int32_t       *pQmfImagAnalysis = qmfImag[ch];

            CalculateSpaceAnalysisQmf(&self->pQmfDomain->QmfDomainIn[ch].fb, inSamples + (ch * offset),
                                      pQmfRealAnalysis, pQmfImagAnalysis);

            if(!isTwoChMode(self->upmixType) && !bypassMode) {
                int32_t i;
                for(i = 0; i < self->qmfBands; i++) {
                    qmfReal[ch][i] = fMult(scaleValueSaturate(qmfReal[ch][i], self->sacInDataHeadroom - (1)),
                                           self->clipProtectGain__FDK);
                    qmfImag[ch][i] = fMult(scaleValueSaturate(qmfImag[ch][i], self->sacInDataHeadroom - (1)),
                                           self->clipProtectGain__FDK);
                }
            }
        }
    ...
    self->qmfInputDelayBufPos = (self->qmfInputDelayBufPos + 1) % self->pc_filterdelay;

    return err;
}

SACDEC_ERROR SpatialDecFeedQMF(spatialDec *self, int32_t **qmfInDataReal, int32_t **qmfInDataImag, const int32_t ts,
                               const int32_t bypassMode, int32_t **qmfReal__FDK, int32_t **qmfImag__FDK,
    ...
    {
        for(ch = 0; ch < numInputChannels; ch++) {
            int32_t *pQmfRealAnalysis = qmfReal__FDK[ch]; /* no delay in blind mode */
            int32_t *pQmfImagAnalysis = qmfImag__FDK[ch];

            /* Write Input data to pQmfRealAnalysis. */
            if(self->bShareDelayWithSBR) {
                FDK_QmfDomain_GetSlot(&self->pQmfDomain->QmfDomainIn[ch], ts + HYBRID_FILTER_DELAY, 0,
                                      MAX_QMF_BANDS_TO_HYBRID, pQmfRealAnalysis, pQmfImagAnalysis, 15 + (1));
                FDK_QmfDomain_GetSlot(&self->pQmfDomain->QmfDomainIn[ch], ts, MAX_QMF_BANDS_TO_HYBRID, self->qmfBands,
                                      pQmfRealAnalysis, pQmfImagAnalysis, 15 + (1));
            }
            else {
                FDK_QmfDomain_GetSlot(&self->pQmfDomain->QmfDomainIn[ch], ts, 0, self->qmfBands, pQmfRealAnalysis,
            ...
            if(!isTwoChMode(self->upmixType) && !bypassMode) {
                int32_t i;
                for(i = 0; i < self->qmfBands; i++) {
                    qmfReal__FDK[ch][i] = fMult(qmfReal__FDK[ch][i], self->clipProtectGain__FDK);
                    qmfImag__FDK[ch][i] = fMult(qmfImag__FDK[ch][i], self->clipProtectGain__FDK);
                }
            }
...
*******************************************************************************/
SACDEC_ERROR SpatialDecHybridAnalysis(spatialDec *self, int32_t **qmfInputReal, int32_t **qmfInputImag,
                                      int32_t **hybOutputReal, int32_t **hybOutputImag, const int32_t ts,
                                      const int32_t numInputChannels) {
    SACDEC_ERROR err = MPS_OK;
    int32_t      ch;

    for(ch = 0; ch < numInputChannels; ch++) /* hybrid filtering for down-mix signals */
    {
        if(self->pConfigCurrent->syntaxFlags & SACDEC_SYNTAX_LD) {
            ...
            if(self->stereoConfigIndex == 3) assert(self->hybridAnalysis[ch].hfMode == 0);
            FDKhybridAnalysisApply(&self->hybridAnalysis[ch], qmfInputReal[ch], qmfInputImag[ch], hybOutputReal[ch],
                                   hybOutputImag[ch]);
        }
    }

    if((self->pConfigCurrent->syntaxFlags & SACDEC_SYNTAX_USAC) && self->residualCoding) {
        self->hybridAnalysis[numInputChannels].hfMode = 0;
        FDKhybridAnalysisApply(&self->hybridAnalysis[numInputChannels], self->qmfResidualReal__FDK[0][0],
                               self->qmfResidualImag__FDK[0][0], self->hybResidualReal__FDK[0],
                               self->hybResidualImag__FDK[0]);
    }

    return err;
}

SACDEC_ERROR SpatialDecCreateX(spatialDec *self, int32_t **hybInputReal, int32_t **hybInputImag, int32_t **pxReal,
                               int32_t **pxImag) {
    SACDEC_ERROR err = MPS_OK;
    int32_t      row;
...
static void M2ParamToKernelMult(int16_t *pKernel, int32_t *Mparam, int32_t *MparamPrev, int32_t *pWidth,
                                int16_t alpha__FDK, int32_t nBands) {
    int32_t pb;

    for(pb = 0; pb < nBands; pb++) {
        int16_t tmp = FX_DBL2FX_SGL(interpolateParameter(alpha__FDK, Mparam[pb], MparamPrev[pb]));
...
SACDEC_ERROR SpatialDecApplyM1_CreateW_Mode212(spatialDec *self, const SPATIAL_BS_FRAME *frame, int32_t **xReal,
    ...
    if(isTsdActive(frame->TsdData)) {
        ...
        TsdGenerateNonTr(self->hybridBands, frame->TsdData, self->TsdTs, vReal[0], vImag[0], vReal[1], vImag[1],
                         &decorrInReal, &decorrInImag);
    }
    /* - Decorrelate */
    res = SpatialDecGetResidualIndex(self, 1);
    if(FDKdecorrelateApply(&self->apDecor[0], decorrInReal, decorrInImag, vReal[1], vImag[1],
    ...
    if(isTsdActive(frame->TsdData)) {
         ...
         * decorrelator output */
        TsdApply(self->hybridBands, frame->TsdData, &self->TsdTs, vReal[0], /* input: v_x */
                 vImag[0], vReal[1], /* input: d_{x,nonTr}; output: d_{x,nonTr} + d_{x,Tr} */
                 vImag[1]);
    }
    ...
    if(self->residualBands[res] > 0) {
        ...
        memcpy(vReal[1], self->hybResidualReal__FDK[res], fixMin(stopBand, self->hybridBands) * sizeof(int32_t));
        memcpy(vImag[1], self->hybResidualImag__FDK[res], fixMin(stopBand, self->hybridBands) * sizeof(int32_t));
    } /* (self->residualBands[res]>0) */

    return err;
}

SACDEC_ERROR SpatialDecApplyM2_Mode212(spatialDec *self, int32_t ps, const int16_t alpha, int32_t **wReal,
...
SACDEC_ERROR SpatialDecApplyM2_Mode212_ResidualsPlusPhaseCoding(spatialDec *self, int32_t ps, const int16_t alpha,
...
SACDEC_ERROR SpatialDecApplyM2(spatialDec *self, int32_t ps, const int16_t alpha, int32_t **wReal, int32_t **wImag,
                               int32_t **hybOutputRealDry, int32_t **hybOutputImagDry, int32_t **hybOutputRealWet,
...

    {
        ...
        int32_t *pWReal, *pWImag, *pHybOutRealDry, *pHybOutImagDry, *pHybOutRealWet, *pHybOutImagWet;

        int16_t pKernel[MAX_HYBRID_BANDS];

        /* The wet signal is added to the dry signal directly in applyM2 if GES and
         * STP are disabled */
        toolsDisabled = ((self->tempShapeConfig == 1) || (self->tempShapeConfig == 2)) ? 0 : 1;
        ...
        memset(hybOutputImagDry[0], 0,
               self->createParams.maxNumOutputChannels * self->createParams.maxNumCmplxHybBands * sizeof(int32_t));
        memset(hybOutputRealDry[0], 0,
               self->createParams.maxNumOutputChannels * self->createParams.maxNumHybridBands * sizeof(int32_t));
        ...
        if(self->phaseCoding == 3) { scale_param_m2 = -(SCALE_DATA_APPLY_M2_PC - 1); }
        ...
        for(row = 0; row < self->numM2rows; row++) {
            ...
            for(col = 0; col < self->numDirektSignals; col++) {
                if(self->pActivM2ParamBands == 0) { /* default setting, calculate all rows and columns */
                    activParamBands = 1;
                }
                else {
                    if(self->pActivM2ParamBands[MAX_M2_INPUT * row + col]) /* table with activ and inactiv
                ...
                if(activParamBands) {
                    ...
                    M2ParamToKernelMult(pKernel, self->M2Real__FDK[row][col], self->M2RealPrev__FDK[row][col],
                                        self->kernels_width, alpha, self->numParameterBands);
                    ...
                    if(1 && (self->phaseCoding != 3)) {
                        ...
                        for(qs = 0; qs < complexHybBands; qs++) {
                            pHybOutRealDry[qs] +=
                                SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWReal[qs], pKernel[qs]), scale_param_m2);
                            pHybOutImagDry[qs] +=
                                SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWImag[qs], pKernel[qs]), scale_param_m2);
                        }

                        M2ParamToKernelMult(pKernel, self->M2Imag__FDK[row][col], self->M2ImagPrev__FDK[row][col],
                                            self->kernels_width, alpha, complexParBands);

                        /* direct signals sign is -1 for qs = 0,2 */
                        pHybOutRealDry[0] += SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWImag[0], pKernel[0]), scale_param_m2);
                        pHybOutImagDry[0] -= SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWReal[0], pKernel[0]), scale_param_m2);

                        pHybOutRealDry[2] += SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWImag[2], pKernel[2]), scale_param_m2);
                        pHybOutImagDry[2] -= SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWReal[2], pKernel[2]), scale_param_m2);

                        /* direct signals sign is +1 for qs = 1,3,4,5,...,complexHybBands */
                        pHybOutRealDry[1] -= SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWImag[1], pKernel[1]), scale_param_m2);
                        pHybOutImagDry[1] += SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWReal[1], pKernel[1]), scale_param_m2);
            ...
            for(; col < self->numVChannels; col++) {
                ...
                if(activParamBands) {
                    ...
                    resBandIndex = self->residualBands[SpatialDecGetResidualIndex(self, col)];
                    ...
                    if(1 && (self->phaseCoding != 3)) {
                        ...
                        if(toolsDisabled) {
                            pHybOutRealDry[0] +=
                                SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWImag[0], pKernel[0]), scale_param_m2);
                            pHybOutImagDry[0] -=
                                SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWReal[0], pKernel[0]), scale_param_m2);

                            pHybOutRealDry[1] -=
                                SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWImag[1], pKernel[1]), scale_param_m2);
                            pHybOutImagDry[1] +=
                                SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWReal[1], pKernel[1]), scale_param_m2);

                            pHybOutRealDry[2] +=
                                SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWImag[2], pKernel[2]), scale_param_m2);
                            pHybOutImagDry[2] -=
                                SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWReal[2], pKernel[2]), scale_param_m2);
                        }
                        else {
                            ...
                            pHybOutReal[0] += SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWImag[0], pKernel[0]), scale_param_m2);
                            pHybOutImag[0] -= SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWReal[0], pKernel[0]), scale_param_m2);

                            if(1 == resHybIndex) {
                                pHybOutReal = &pHybOutRealWet[0];
                                pHybOutImag = &pHybOutImagWet[0];
                            }
                            pHybOutReal[1] -= SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWImag[1], pKernel[1]), scale_param_m2);
                            pHybOutImag[1] += SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWReal[1], pKernel[1]), scale_param_m2);

                            if(2 == resHybIndex) {
                                pHybOutReal = &pHybOutRealWet[0];
                                pHybOutImag = &pHybOutImagWet[0];
                            }
                            pHybOutReal[2] += SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWImag[2], pKernel[2]), scale_param_m2);
                            pHybOutImag[2] -= SAC_DEC_APPLY_M2_SCALE(fMultDiv2(pWReal[2], pKernel[2]), scale_param_m2);
                        }
            ...
            if(self->phaseCoding == 3) {
                scaleValuesSaturate(pHybOutRealDry, complexHybBands, SCALE_PARAM_M2_212_PRED + SCALE_DATA_APPLY_M2_PC);
                scaleValuesSaturate(pHybOutImagDry, complexHybBands, SCALE_PARAM_M2_212_PRED + SCALE_DATA_APPLY_M2_PC);
...
SACDEC_ERROR SpatialDecSynthesis(spatialDec *self, const int32_t ts, int32_t **hybOutputReal, int32_t **hybOutputImag,
                                 PCM_MPS *timeOut, const int32_t numInputChannels,
    ...
    PCM_MPS *pTimeOut__FDK = &timeOut[stride * self->pQmfDomain->globalConf.nBandsSynthesis * ts];
    int32_t pQmfReal[QMF_MAX_SYNTHESIS_BANDS];
    int32_t pQmfImag[QMF_MAX_SYNTHESIS_BANDS];
    ...
    for(ch = 0; ch < self->numOutputChannelsAT; ch++) {
        if(self->pConfigCurrent->syntaxFlags & SACDEC_SYNTAX_LD) {
            ...
            FDKhybridSynthesisApply(&self->hybridSynthesis[ch], hybOutputReal[ch], hybOutputImag[ch], pQmfReal,
                                    pQmfImag);
        }
        ...
        int32_t outCh = FDK_chMapDescr_getMapValue(mapDescr, mapChannel(self, ch), self->numOutputChannelsAT);

        {
            if(self->stereoConfigIndex == 3) {
                ...
                assert((self->pQmfDomain->QmfDomainOut[outCh].fb.outGain_m == (int32_t)0x80000000) &&
                       (self->pQmfDomain->QmfDomainOut[outCh].fb.outGain_e == 0));
                FDK_QmfDomain_GetWorkBuffer(&self->pQmfDomain->QmfDomainIn[outCh], ts, &pWorkBufReal, &pWorkBufImag);
                assert(self->qmfBands <= self->pQmfDomain->QmfDomainIn[outCh].workBuf_nBands);
                for(i = 0; i < self->qmfBands; i++) {
                ...
                self->pQmfDomain->QmfDomainIn[outCh].scaling.lb_scale -= self->clipProtectGainSF__FDK;

                self->pQmfDomain->QmfDomainIn[outCh].scaling.lb_scale -= (1);
            }
            else {
                /* Call the QMF synthesis for dry. */
                err = CalculateSpaceSynthesisQmf(&self->pQmfDomain->QmfDomainOut[outCh], pQmfReal, pQmfImag, stride,
    ...
    for(row = 0; row < self->numM2rows; row++) {
        for(col = 0; col < self->numVChannels; col++) {
            ...
            if(0 || (self->phaseCoding == 3)) {
                memcpy(self->M2ImagPrev__FDK[row][col], self->M2Imag__FDK[row][col], complexParBands * sizeof(int32_t));
            }
        }
    }

    /* buffer phase */
    memcpy(self->PhasePrevLeft__FDK, self->PhaseLeft__FDK, self->numParameterBands * sizeof(int32_t));
    memcpy(self->PhasePrevRight__FDK, self->PhaseRight__FDK, self->numParameterBands * sizeof(int32_t));
}
...
static int32_t interp_angle__FDK(int32_t angle1, int32_t angle2, int16_t alpha, int32_t pi_x2) {
 ...
 *
 */
void SpatialDecApplyPhase(spatialDec *self, int16_t alpha__FDK, int32_t lastSlotOfParamSet) {
    int32_t pb, qs;
    int32_t ppb[MAX_PARAMETER_BANDS * 4]; /* left real, imag - right real, imag interleaved */

    const int32_t pi_x2 = PIx2__IPD;
    for(pb = 0; pb < self->numParameterBands; pb++) {
        int32_t pl, pr;

        pl = interp_angle__FDK(self->PhasePrevLeft__FDK[pb], self->PhaseLeft__FDK[pb], alpha__FDK, pi_x2);
        pr = interp_angle__FDK(self->PhasePrevRight__FDK[pb], self->PhaseRight__FDK[pb], alpha__FDK, pi_x2);
    ...
    for(qs = 2; qs >= 0; qs--) {
        ...
        if(qs == 1) /* sign[qs] >= 0 */
        {
            cplxMultDiv2(&out_re, &out_im, *Dry_real0, *Dry_imag0, ppb[4 * pb + 0], ppb[4 * pb + 1]);
            out_re <<= PHASE_SCALE - 1;
            out_im <<= PHASE_SCALE - 1;
            *Dry_real0++ = out_re;
            *Dry_imag0++ = out_im;

            cplxMultDiv2(&out_re, &out_im, *Dry_real1, *Dry_imag1, ppb[4 * pb + 2], ppb[4 * pb + 3]);
            out_re <<= PHASE_SCALE - 1;
            out_im <<= PHASE_SCALE - 1;
            *Dry_real1++ = out_re;
            *Dry_imag1++ = out_im;
