...
typedef enum
{
    ...
    MPEGS_START_STOP = 3 /*!< Indicates if data segment begins and ends a data block. */
} MPEGS_ANCSTARTSTOP;
...
typedef enum
{
    MPEGS_SYNC_LOST = 0,    /*!< Indicates lost sync because of current discontinuity. */
...
typedef enum
{
    ...
    MPEGS_INIT_ENFORCE_REINIT = 0x00000001, /*!< indicate complete initialization */

    MPEGS_INIT_CHANGE_OUTPUT_MODE = 0x00000010,         /*!< indicate change of the output mode */
    MPEGS_INIT_CHANGE_PARTIALLY_COMPLEX = 0x00000020,   /*!< indicate change of low power/high quality */
    MPEGS_INIT_CHANGE_TIME_FREQ_INTERFACE = 0x00000040, /*!< indicate change of qmf/time interface */
    MPEGS_INIT_CHANGE_HEADER = 0x00000080,              /*!< indicate change of header */

    MPEGS_INIT_ERROR_PAYLOAD = 0x00000100, /*!< indicate payload/ancType/ancStartStop error */

    MPEGS_INIT_BS_INTERRUPTION = 0x00001000, /*!< indicate bitstream interruption  */
    MPEGS_INIT_CLEAR_HISTORY = 0x00002000,   /*!< indicate that all states shall be cleared */
    ...
    MPEGS_INIT_CHANGE_BYPASS_MODE = 0x01000000, /*!< indicate change of bypass mode */

    /* Re-initialization needed, currently not used */
    MPEGS_INIT_ERROR_ANC_TYPE = 0x10000000,     /*!< indicate ancType error*/
    MPEGS_INIT_ERROR_ANC_STARTSTOP = 0x20000000 /*!< indicate ancStartStop error */
} MPEGS_INIT_FLAGS;
...
SACDEC_ERROR
static sscCheckOutOfBand(const SPATIAL_SPECIFIC_CONFIG *pSsc, const int32_t coreCodec, const int32_t sampleRate,
 ...
 **/
static int32_t mpegSurroundDecoder_GetNrOfQmfBands(const SPATIAL_SPECIFIC_CONFIG *pSsc, uint32_t sampleRate) {
    uint32_t samplingFrequency = sampleRate;
    int32_t  qmfBands = 64;
        ...
        switch(pSsc->coreCodec) {
            case AOT_USAC:
                if((pSsc->stereoConfigIndex == 3)) {
                    ...
                    assert((pSsc->coreSbrFrameLengthIndex >= 2) && (pSsc->coreSbrFrameLengthIndex <= 4));
     ...
     * Chapter 6.3.3 */
    if(samplingFrequency < 27713) { qmfBands = 32; }
    if(samplingFrequency > 55426) { qmfBands = 128; }
 ...
 **/
static int32_t mpegSurroundDecoder_CalcInitFlags(SPATIAL_SPECIFIC_CONFIG *pSsc1, SPATIAL_SPECIFIC_CONFIG *pSsc2,
                                                 int32_t upmixTypeFlag, int32_t binauralQualityFlag,
                                                 int32_t partiallyComplexFlag, int32_t *ctrlFlags) {
    /* Analyse core coder */
    if(pSsc1->coreCodec != pSsc2->coreCodec) {
        ...
        *ctrlFlags |= MASK_MPEGS_INIT_ALL_PARAMS;
        ...
        if((upmixTypeFlag) || (partiallyComplexFlag) || (pSsc1->treeConfig != pSsc2->treeConfig) ||
           (pSsc1->samplingFreq != pSsc2->samplingFreq) || (pSsc1->bsFixedGainDMX != pSsc2->bsFixedGainDMX)) {
            *ctrlFlags |= MPEGS_INIT_STATES_SYN_QMF_FILTER;
        }

        /* Analyse elements for initialization of decorrelator */
        if((upmixTypeFlag) || (partiallyComplexFlag) || (pSsc1->treeConfig != pSsc2->treeConfig) ||
           (pSsc1->samplingFreq != pSsc2->samplingFreq) || (pSsc1->decorrConfig != pSsc2->decorrConfig)) {
            *ctrlFlags |= MPEGS_INIT_STATES_DECORRELATOR;
        }

        /* Analyse elements for initialization of m1 and m2 calculation */
        if((upmixTypeFlag) || (binauralQualityFlag) || (pSsc1->treeConfig != pSsc2->treeConfig) ||
           (pSsc1->samplingFreq != pSsc2->samplingFreq))
        ...
        if((upmixTypeFlag) || (binauralQualityFlag) || (partiallyComplexFlag) ||
           (pSsc1->samplingFreq != pSsc2->samplingFreq) || (pSsc1->nTimeSlots != pSsc2->nTimeSlots)) {
            *ctrlFlags |= MPEGS_INIT_STATES_REVERB;
        }

        /* Reset previous frame data whenever the config changes */
        if(*ctrlFlags & MPEGS_INIT_CONFIG) { *ctrlFlags |= MPEGS_INIT_STATES_PARAM; }
    }
 ...
 **/
static void updateMpegSurroundDecoderStatus(CMpegSurroundDecoder *pMpegSurroundDecoder, int32_t initFlags,
                                            MPEGS_SYNCSTATE fOnSync, MPEGS_ANCSTARTSTOP ancStartStopPrev) {
    pMpegSurroundDecoder->initFlags[pMpegSurroundDecoder->bsFrameDecode] |= initFlags;
    if((pMpegSurroundDecoder->mpegSurroundSscIsGlobalCfg != 0) &&
       (pMpegSurroundDecoder->fOnSync[pMpegSurroundDecoder->bsFrameDecode] >= MPEGS_SYNC_FOUND) &&
       (fOnSync < MPEGS_SYNC_FOUND)) {
        pMpegSurroundDecoder->fOnSync[pMpegSurroundDecoder->bsFrameDecode] = MPEGS_SYNC_FOUND;
    }
    else { pMpegSurroundDecoder->fOnSync[pMpegSurroundDecoder->bsFrameDecode] = fOnSync; }
    pMpegSurroundDecoder->ancStartStopPrev = ancStartStopPrev;
}

static SACDEC_ERROR mpegSurroundDecoder_Create(CMpegSurroundDecoder **pMpegSurroundDecoder, int32_t stereoConfigIndex,
                                               HANDLE_FDK_QMF_DOMAIN pQmfDomain);

SAC_INSTANCE_AVAIL
mpegSurroundDecoder_IsFullMpegSurroundDecoderInstanceAvailable(CMpegSurroundDecoder *pMpegSurroundDecoder) {
    SAC_INSTANCE_AVAIL instanceAvailable = SAC_INSTANCE_NOT_FULL_AVAILABLE;

    if(pMpegSurroundDecoder->pSpatialDec != NULL) { instanceAvailable = SAC_INSTANCE_FULL_AVAILABLE; }

    return instanceAvailable;
}

SACDEC_ERROR mpegSurroundDecoder_Open(CMpegSurroundDecoder **pMpegSurroundDecoder, int32_t stereoConfigIndex,
                                      HANDLE_FDK_QMF_DOMAIN pQmfDomain) {
    SACDEC_ERROR error;

    error = mpegSurroundDecoder_Create(pMpegSurroundDecoder, stereoConfigIndex, pQmfDomain);
 ...
 */
static SACDEC_ERROR check_UParam_Build_DecConfig(SPATIALDEC_PARAM const        *pUserParams,
                                                 SPATIAL_DEC_CONFIG const      *pDecConfig,
                                                 const SPATIAL_SPECIFIC_CONFIG *pSsc,
                                                 SPATIAL_DEC_UPMIX_TYPE        *pUpmixType) {
    ...
    if(pUserParams->blindEnable == 1) { return MPS_UNSUPPORTED_CONFIG; }
    {
        switch((SAC_DEC_OUTPUT_MODE)pUserParams->outputMode) {
            case SACDEC_OUT_MODE_NORMAL:
                if(maxNumOutChannels >= outChannels) { *pUpmixType = UPMIX_TYPE_NORMAL; }
                else {
            ...
            case SACDEC_OUT_MODE_STEREO:
                if(dmxChannels == 1) {
                    if(outChannels == 2) { *pUpmixType = UPMIX_TYPE_NORMAL; }
                }
                else { *pUpmixType = UPMIX_TYPE_BYPASS; }
                break;
            case SACDEC_OUT_MODE_6CHANNEL:
                if(outChannels > 6) {
                    { *pUpmixType = UPMIX_TYPE_BYPASS; }
                }
                else { *pUpmixType = UPMIX_TYPE_NORMAL; }
                break;
            default:
                return MPS_UNSUPPORTED_CONFIG;
        }
 ...
 **/
static SACDEC_ERROR initMpegSurroundDecoder(CMpegSurroundDecoder *pMpegSurroundDecoder) {
    SACDEC_ERROR err;
    ...
    SPATIAL_SPECIFIC_CONFIG *const pSSCinput = &pMpegSurroundDecoder->spatialSpecificConfigBackup;
    SPATIAL_SPECIFIC_CONFIG *const pSSCtarget =
        &pMpegSurroundDecoder->spatialSpecificConfig[pMpegSurroundDecoder->bsFrameDecode];
    initFlagsDec = pMpegSurroundDecoder->initFlags[pMpegSurroundDecoder->bsFrameDecode];
    ...
    err = check_UParam_Build_DecConfig(&pMpegSurroundDecoder->mpegSurroundUserParams, &pMpegSurroundDecoder->decConfig,
                                       pSSCinput, &pMpegSurroundDecoder->upmixType);
    if(err != MPS_OK) goto bail;

    /* init config */
    if(initFlagsDec & MPEGS_INIT_CHANGE_HEADER) { initFlags |= MPEGS_INIT_CONFIG; }
    /* init all states */
    if(initFlagsDec & MPEGS_INIT_CLEAR_HISTORY) { initFlags |= MASK_MPEGS_INIT_ALL_STATES; }
    if(initFlagsDec & MPEGS_INIT_CHANGE_CONCEAL_PARAMS) { initFlags |= MPEGS_INIT_PARAMS_ERROR_CONCEALMENT; }
    ...
    if(initFlagsDec & MPEGS_INIT_ENFORCE_REINIT) {
        ...
        mpegSurroundDecoder_CalcInitFlags(pSSCtarget, pSSCinput,
                                          (upmixTypeCurr != pMpegSurroundDecoder->upmixType), /* upmixType changed */
                                          0, (initFlagsDec & MPEGS_INIT_CHANGE_PARTIALLY_COMPLEX) ? 1 : 0, &initFlags);
    }

    {
        ...
        err = FDK_SpatialDecInit(pMpegSurroundDecoder->pSpatialDec,
                                 &pMpegSurroundDecoder->bsFrames[pMpegSurroundDecoder->bsFrameDecode], pSSCtarget,
                                 nrOfQmfBands, pMpegSurroundDecoder->upmixType,
...
bail:
    pMpegSurroundDecoder->initFlags[pMpegSurroundDecoder->bsFrameDecode] = initFlagsDec;
    return err;
}
 ...
 * \brief Init MPEG Surround decoder.
 **/
SACDEC_ERROR mpegSurroundDecoder_Init(CMpegSurroundDecoder *pMpegSurroundDecoder) {
...
static SACDEC_ERROR mpegSurroundDecoder_Create(CMpegSurroundDecoder **pMpegSurroundDecoder, int32_t stereoConfigIndex,
    ...
    static const SPATIAL_DEC_CONFIG decConfig = {(CFG_LEVEL)(0), EXT_HQ_ONLY, OUTPUT_CHANNELS_DEFAULT, BINAURAL_NONE};

    if(*pMpegSurroundDecoder == NULL) {
        FDK_ALLOCATE_MEMORY_1D(*pMpegSurroundDecoder, 1, CMpegSurroundDecoder)

        for(int32_t i = 0; i < 1; i++) {
            err = SpatialDecCreateBsFrame(&(*pMpegSurroundDecoder)->bsFrames[i], &(*pMpegSurroundDecoder)->llState);
            if(err != MPS_OK) {
    ...
    else { sacDec = *pMpegSurroundDecoder; }
    ...
    updateMpegSurroundDecoderStatus(sacDec, MPEGS_INIT_ENFORCE_REINIT, MPEGS_SYNC_LOST, MPEGS_STOP);
...
bail:
    if(sacDec != NULL) { mpegSurroundDecoder_Close(sacDec); }
    *pMpegSurroundDecoder = NULL;
    if(err == MPS_OK) { return MPS_OUTOFMEMORY; }
    else { return err; }
}
...
SACDEC_ERROR mpegSurroundDecoder_Config(CMpegSurroundDecoder *pMpegSurroundDecoder, HANDLE_FDK_BITSTREAM hBs,
    ...
    SPATIAL_SPECIFIC_CONFIG *pSsc = &pMpegSurroundDecoder->spatialSpecificConfigBackup;
    ...
    switch(coreCodec) {
        ...
        case AOT_USAC:
            if(configMode == AC_CM_DET_CFG_CHANGE) {
                ...
                err = SpatialDecParseMps212Config(hBs, &spatialSpecificConfig, samplingRate, coreCodec,
                                                  stereoConfigIndex, coreSbrFrameLengthIndex);
                pSsc = &spatialSpecificConfig;
            }
            else {
                err = SpatialDecParseMps212Config(hBs, &pMpegSurroundDecoder->spatialSpecificConfigBackup, samplingRate,
                                                  coreCodec, stereoConfigIndex, coreSbrFrameLengthIndex);
            }
            break;
        case AOT_ER_AAC_ELD:
        case AOT_ER_AAC_LD:
            if(configMode == AC_CM_DET_CFG_CHANGE) {
                /* In config detection mode write spatial specific config parameters
                 * into temporarily allocated structure */
                err = SpatialDecParseSpecificConfig(hBs, &spatialSpecificConfig, configBytes, coreCodec);
                pSsc = &spatialSpecificConfig;
            }
            else {
                err = SpatialDecParseSpecificConfig(hBs, &pMpegSurroundDecoder->spatialSpecificConfigBackup,
    ...
    if(configMode & AC_CM_DET_CFG_CHANGE) { return err; }

    if(configMode & AC_CM_ALLOC_MEM) {
        if(*configChanged) {
            err = mpegSurroundDecoder_Open(&pMpegSurroundDecoder, stereoConfigIndex, NULL);
            if(err) { return err; }
        }
    }

    {
        SPATIAL_SPECIFIC_CONFIG *sscParse =
            &pMpegSurroundDecoder->spatialSpecificConfig[pMpegSurroundDecoder->bsFrameParse];

        if(FDK_SpatialDecCompareSpatialSpecificConfigHeader(&pMpegSurroundDecoder->spatialSpecificConfigBackup,
                                                            sscParse)) {
            pMpegSurroundDecoder->initFlags[pMpegSurroundDecoder->bsFrameParse] |= MPEGS_INIT_CHANGE_HEADER;
 ...
 **/
static MPEGS_OPMODE mpegSurroundOperationMode(CMpegSurroundDecoder *pMpegSurroundDecoder, int32_t mpsDataBits) {
    MPEGS_OPMODE mode;

    {
        if((mpsDataBits > 0) && (pMpegSurroundDecoder->mpegSurroundUserParams.blindEnable == 0)) {
            ...
            updateMpegSurroundDecoderStatus(pMpegSurroundDecoder, MPEGS_INIT_ERROR_PAYLOAD, MPEGS_SYNC_LOST,
                                            MPEGS_STOP);
        }
    }
...
static SACDEC_ERROR sscParseCheck(const SPATIAL_SPECIFIC_CONFIG *pSsc) {
    ...
    if((pSsc->treeConfig < 0) || (pSsc->treeConfig > 7)) { return MPS_PARSE_ERROR; }

    if((pSsc->quantMode < 0) || (pSsc->quantMode > 2)) { return MPS_PARSE_ERROR; }
 ...
 * \return  error code
 */
SACDEC_ERROR checkTimeSlots(int32_t frameLength, int32_t qmfBands, int32_t timeSlots) {
    int32_t len;
    int32_t maxFrameLength;
    ...
    else { return MPS_PARSE_ERROR; }

    len = frameLength;

    while(len <= maxFrameLength) {
        if(len == timeSlots * qmfBands) { return MPS_OK; }
        len += frameLength;
    }
    return MPS_PARSE_ERROR;
}
 ...
 * \return  MPS_OK on sucess, and else on failure.
 */
static SACDEC_ERROR sscCheckInBand(SPATIAL_SPECIFIC_CONFIG *pSsc, int32_t frameLength, int32_t sampleRate) {
    SACDEC_ERROR err = MPS_OK;
    int32_t      qmfBands;
    ...
    if(pSsc->samplingFreq != sampleRate) { err = MPS_PARSE_ERROR /* MPEGSDEC_SSC_PARSE_ERROR */; }

    qmfBands = mpegSurroundDecoder_GetNrOfQmfBands(pSsc, pSsc->samplingFreq);

    if(checkTimeSlots(frameLength, qmfBands, pSsc->nTimeSlots) != MPS_OK) { err = MPS_PARSE_ERROR; }
...
mpegSurroundDecoder_ConfigureQmfDomain(CMpegSurroundDecoder *pMpegSurroundDecoder, SAC_INPUT_CONFIG sac_dec_interface,
    ...
    if(pMpegSurroundDecoder == NULL) { return MPS_INVALID_HANDLE; }
    ...
    if(pMpegSurroundDecoder->mpegSurroundSscIsGlobalCfg) {
        ...
        if(sac_dec_interface == SAC_INTERFACE_TIME) {
            ...
            pGC->nBandsAnalysis_requested =
                mpegSurroundDecoder_GetNrOfQmfBands(pSSC, coreSamplingRate); /* coreSamplingRate == outputSamplingRate
                                                                                for SAC_INTERFACE_TIME */
            pGC->nBandsSynthesis_requested = pGC->nBandsAnalysis_requested;
            pGC->nInputChannels_requested =
                fMax((uint32_t)pSSC->nInputChannels, (uint32_t)pGC->nInputChannels_requested);
        }
        pGC->nOutputChannels_requested =
            fMax((uint32_t)pSSC->nOutputChannels, (uint32_t)pGC->nOutputChannels_requested);
    }
    else {
        if(sac_dec_interface == SAC_INTERFACE_TIME) {
            /* For SAC_INTERFACE_QMF these parameters are set by SBR. */
            pGC->nBandsAnalysis_requested =
                mpegSurroundDecoder_GetNrOfQmfBands(NULL, coreSamplingRate); /* coreSamplingRate == outputSamplingRate
                                                                                for SAC_INTERFACE_TIME */
            pGC->nBandsSynthesis_requested = pGC->nBandsAnalysis_requested;
            pGC->nInputChannels_requested = pMpegSurroundDecoder->pSpatialDec->createParams.maxNumInputChannels;
        }
        pGC->nOutputChannels_requested = pMpegSurroundDecoder->pSpatialDec->createParams.maxNumOutputChannels;
    }
    pGC->nQmfProcBands_requested = 64;
    pGC->nQmfProcChannels_requested = fMin((int32_t)pGC->nInputChannels_requested,
 ...
 */
SACDEC_ERROR
sscCheckOutOfBand(const SPATIAL_SPECIFIC_CONFIG *pSsc, const int32_t coreCodec, const int32_t sampleRate,
                  const int32_t frameSize) {
    assert(pSsc != NULL);
    int32_t qmfBands = 0;

    /* check ssc for parse errors */
    if(sscParseCheck(pSsc) != MPS_OK) { return MPS_PARSE_ERROR; }
    ...
    switch(coreCodec) {
        ...
        case AOT_DRM_USAC:
            ...
            if(pSsc->samplingFreq >= 55426) { return MPS_PARSE_ERROR; }
            break;
        case AOT_ER_AAC_LD:
        case AOT_ER_AAC_ELD:
            /* core fs and mps fs must match */
            if(pSsc->samplingFreq != sampleRate) { return MPS_PARSE_ERROR; }

            /* ISO/IEC 14496-3:2009 FDAM 3: Chapter 1.5.2.3, Levels for the Low Delay
             * AAC v2 profile */
            if(pSsc->samplingFreq > 48000) { return MPS_PARSE_ERROR; }

            qmfBands = mpegSurroundDecoder_GetNrOfQmfBands(pSsc, pSsc->samplingFreq);
            switch(frameSize) {
                case 480:
                    if(!((qmfBands == 32) && (pSsc->nTimeSlots == 15))) { return MPS_PARSE_ERROR; }
                    break;
                case 960:
                    if(!((qmfBands == 64) && (pSsc->nTimeSlots == 15))) { return MPS_PARSE_ERROR; }
                    break;
                case 512:
                    ...
                    break;
                case 1024:
                    if(!((qmfBands == 64) && (pSsc->nTimeSlots == 16))) { return MPS_PARSE_ERROR; }
                    break;
                default:
                    return MPS_PARSE_ERROR;
            }
 ...
 **/
int32_t mpegSurroundDecoder_ParseNoHeader(CMpegSurroundDecoder *pMpegSurroundDecoder, HANDLE_FDK_BITSTREAM hBs,
                                          int32_t *pMpsDataBits, int32_t fGlobalIndependencyFlag) {
    SACDEC_ERROR             err = MPS_OK;
    SPATIAL_SPECIFIC_CONFIG *sscParse;
    int32_t                  bitsAvail, numSacBits;

    if(pMpegSurroundDecoder == NULL || hBs == NULL) { return MPS_INVALID_HANDLE; }

    sscParse = &pMpegSurroundDecoder->spatialSpecificConfig[pMpegSurroundDecoder->bsFrameParse];
    ...
    if(bitsAvail <= 0) { err = MPS_PARSE_ERROR; }
    else {
        err = SpatialDecParseFrameData(pMpegSurroundDecoder->pSpatialDec,
                                       &pMpegSurroundDecoder->bsFrames[pMpegSurroundDecoder->bsFrameParse], hBs,
                                       sscParse, (UPMIXTYPE)pMpegSurroundDecoder->upmixType, fGlobalIndependencyFlag);
        if(err == MPS_OK) { pMpegSurroundDecoder->bsFrames[pMpegSurroundDecoder->bsFrameParse].newBsData = 1; }
    }

    numSacBits = bitsAvail - (int32_t)FDKgetValidBits(hBs);

    if(numSacBits > bitsAvail) {
        pMpegSurroundDecoder->bsFrames[pMpegSurroundDecoder->bsFrameParse].newBsData = 0;
        err = MPS_PARSE_ERROR;
    }
 ...
 **/
static int32_t isValidAncType(CMpegSurroundDecoder *pMpegSurroundDecoder, int32_t ancType) {
    int32_t ret = 1;

    if((ancType != MPEGS_ANCTYPE_HEADER_AND_FRAME) && (ancType != MPEGS_ANCTYPE_FRAME)) { ret = 0; }

    if(ret == 0) {
        updateMpegSurroundDecoderStatus(pMpegSurroundDecoder, MPEGS_INIT_ERROR_PAYLOAD, MPEGS_SYNC_LOST, MPEGS_STOP);
    }
 ...
 **/
static int32_t isValidAncStartStop(CMpegSurroundDecoder *pMpegSurroundDecoder, int32_t ancStartStop) {
    int32_t ret = 1;
    ...
    else { pMpegSurroundDecoder->ancStartStopPrev = (MPEGS_ANCSTARTSTOP)ancStartStop; }
            ...
            if(isValidAncStartStop(pMpegSurroundDecoder, ancStartStop)) {
                switch(ancStartStop) {
                    case MPEGS_START:
                        ...
                        for(int32_t i = 0; i < mpsDataBits / 8; i++) {
                    ...
                    case MPEGS_STOP:
                        /* Assuming that core coder frame size (AAC) is smaller than MPS
                           coder frame size. Save audio data for next frame. */
                        if((pMpegSurroundDecoder->mpsDataBits + mpsDataBits) > MPS_DATA_BUFFER_SIZE * 8) {
                            err = MPS_NOTOK;
                            goto bail;
                        }
                        for(int32_t i = 0; i < mpsDataBits / 8; i++) {
                            pMpegSurroundDecoder->mpsData[(pMpegSurroundDecoder->mpsDataBits / 8) + i] =
                                FDKreadBits(hBs, 8);
                        }
                        pMpegSurroundDecoder->mpsDataBits += mpsDataBits;
                        FDKinitBitStream(&mpsBsData, pMpegSurroundDecoder->mpsData, MAX_BUFSIZE_BYTES,
                ...
                if((ancStartStop == MPEGS_STOP) || (ancStartStop == MPEGS_START_STOP)) {
                    switch(ancType) {
                        case MPEGS_ANCTYPE_HEADER_AND_FRAME: {
                            int32_t                 parseResult, bitsRead;
                            ...
                            bitsRead = (int32_t)FDKgetValidBits(hMpsBsData);

                            err = SpatialDecParseSpecificConfigHeader(
                                hMpsBsData, &pMpegSurroundDecoder->spatialSpecificConfigBackup, coreCodec,
                                pMpegSurroundDecoder->upmixType);

                            bitsRead = (bitsRead - (int32_t)FDKgetValidBits(hMpsBsData));
                            ...
                            if(parseResult < 0) {
                                 ...
                                 * trouble) */
                                err = sscCheckInBand(&pMpegSurroundDecoder->spatialSpecificConfigBackup, frameSize,
                                                     sampleRate);
                            }
                            if(err != MPS_OK) {
                                pMpegSurroundDecoder->spatialSpecificConfigBackup = spatialSpecificConfigTmp;
                                break;
                            }
                            ...
                            if(FDK_SpatialDecCompareSpatialSpecificConfigHeader(
                                   &pMpegSurroundDecoder->spatialSpecificConfigBackup, sscParse) == MPS_UNEQUAL_SSC) {
                        ...
                        case MPEGS_ANCTYPE_FRAME:
                            ...
                            if(pMpegSurroundDecoder->fOnSync[pMpegSurroundDecoder->bsFrameParse] >= MPEGS_SYNC_FOUND) {
                                int32_t nbits = 0, bitsAvail;
                                ...
                                if(bitsAvail <= 0) { err = MPS_PARSE_ERROR; }
                                    ...
                                    err = SpatialDecParseFrameData(
                                        pMpegSurroundDecoder->pSpatialDec, bsFrame, hMpsBsData, sscParse,
                                        (UPMIXTYPE)pMpegSurroundDecoder->upmixType, fGlobalIndependencyFlag);
                                    if(err == MPS_OK) { bsFrame->newBsData = 1; }
                                }

                                nbits = bitsAvail - (int32_t)FDKgetValidBits(hMpsBsData);

                                if((nbits > bitsAvail) || (nbits > pMpegSurroundDecoder->mpsDataBits) ||
                                   (pMpegSurroundDecoder->mpsDataBits > nbits + 7 && !IS_LOWDELAY(coreCodec))) {
                                    bsFrame->newBsData = 0;
                                    err = MPS_PARSE_ERROR;
                                    break;
                                }
                    ...
                    if(err == MPS_OK) { pMpegSurroundDecoder->ancStartStopPrev = ancStartStop; }
                    else {
                        updateMpegSurroundDecoderStatus(pMpegSurroundDecoder, MPEGS_INIT_ERROR_PAYLOAD, MPEGS_SYNC_LOST,
                                                        MPEGS_STOP);
...
bail:
    ...
    *pMpsDataBits -= (mpsBsBits - (int32_t)FDKgetValidBits(hBs));
...
int32_t mpegSurroundDecoder_Apply(CMpegSurroundDecoder *pMpegSurroundDecoder, PCM_MPS *input, PCM_MPS *pTimeData,
                                  ...
                                  AUDIO_CHANNEL_TYPE_t channelType[], uint8_t channelIndices[],
    ...
    uint32_t     initControlFlags = 0, controlFlags = 0;
    int32_t      timeDataRequiredSize = 0;
    int32_t      newData;
    ...
    if(!FDK_chMapDescr_isValid(mapDescr)) { return MPS_INVALID_HANDLE; }

    if((*nChannels <= 0) || (*nChannels > 2)) { return MPS_NOTOK; }
    ...
    *outDataHeadroom = (int32_t)(8);
    ...
    pMpegSurroundDecoder->pSpatialDec->pConfigCurrent =
        &pMpegSurroundDecoder->spatialSpecificConfig[pMpegSurroundDecoder->bsFrameDecode];
    newData = pMpegSurroundDecoder->bsFrames[pMpegSurroundDecoder->bsFrameParse].newBsData;
    ...
    switch(mpegSurroundOperationMode(pMpegSurroundDecoder, 1000)) {
        case MPEGS_OPMODE_MPS_PAYLOAD:
            ...
            if(err == MPS_OK) {
                if((pMpegSurroundDecoder->fOnSync[pMpegSurroundDecoder->bsFrameDecode] != MPEGS_SYNC_COMPLETE) &&
                   (pMpegSurroundDecoder->bsFrames[pMpegSurroundDecoder->bsFrameDecode].bsIndependencyFlag == 1)) {
                    /* We got a valid header and independently decodeable frame data.
                        -> Go to the next sync level and start processing. */
                    pMpegSurroundDecoder->fOnSync[pMpegSurroundDecoder->bsFrameDecode] = MPEGS_SYNC_COMPLETE;
                }
            }
            else {
     ...
     */
    if(pMpegSurroundDecoder->mpegSurroundUserParams.bypassMode) { controlFlags |= MPEGS_BYPASSMODE; }

    if(pMpegSurroundDecoder->initFlags[pMpegSurroundDecoder->bsFrameDecode]) {
        int32_t startWithDfltCfg = 0;
        /*
         * Init with a default configuration if we came here and are still not
         * initialized.
         */
        if(pMpegSurroundDecoder->initFlags[pMpegSurroundDecoder->bsFrameDecode] & MPEGS_INIT_ENFORCE_REINIT) {
            /* Get default spatial specific config */
            if(FDK_SpatialDecInitDefaultSpatialSpecificConfig(
                   &pMpegSurroundDecoder->spatialSpecificConfigBackup, coreCodec, *nChannels, sampleRate,
                   *frameSize / mpegSurroundDecoder_GetNrOfQmfBands(NULL, sampleRate),
            ...
            if(FDK_SpatialDecCompareSpatialSpecificConfigHeader(
                   ...
                   &pMpegSurroundDecoder->spatialSpecificConfig[pMpegSurroundDecoder->bsFrameDecode]) ==
               MPS_UNEQUAL_SSC) {
                pMpegSurroundDecoder->initFlags[pMpegSurroundDecoder->bsFrameDecode] |= MPEGS_INIT_CHANGE_HEADER;
                SpatialDecInitParserContext(pMpegSurroundDecoder->pSpatialDec);
            }
        ...
        if(startWithDfltCfg) {
            ...
            pMpegSurroundDecoder->fOnSync[pMpegSurroundDecoder->bsFrameDecode] = MPEGS_SYNC_LOST;
        }
    ...
    timeDataRequiredSize = (timeDataFrameSize * pMpegSurroundDecoder->pSpatialDec->numOutputChannelsAT *
    ...
    if((pMpegSurroundDecoder->pSpatialDec->pConfigCurrent->syntaxFlags & SACDEC_SYNTAX_USAC) &&
       (pMpegSurroundDecoder->pSpatialDec->stereoConfigIndex > 1)) {
        ...
        int32_t timeDataFrameSizeOut =
            (timeDataFrameSize * pMpegSurroundDecoder->pQmfDomain->globalConf.nBandsSynthesis) /
            pMpegSurroundDecoder->pQmfDomain->globalConf.nBandsAnalysis;
     ...
     * Process MPEG Surround Audio
     */
    err = SpatialDecApplyFrame(pMpegSurroundDecoder->pSpatialDec,
                               &pMpegSurroundDecoder->bsFrames[pMpegSurroundDecoder->bsFrameDecode],
                               pMpegSurroundDecoder->mpegSurroundUseTimeInterface ? INPUTMODE_TIME : INPUTMODE_QMF_SBR,
                               TDinput, NULL, NULL, pTimeOut, *frameSize, &controlFlags, *nChannels, mapDescr);
    *nChannels = pMpegSurroundDecoder->pSpatialDec->numOutputChannelsAT;

    if(err != MPS_OK) { /* A fatal error occured. Go back to start and try again: */
        updateMpegSurroundDecoderStatus(pMpegSurroundDecoder, MPEGS_INIT_ENFORCE_REINIT, MPEGS_SYNC_LOST, MPEGS_STOP);
        *frameSize = 0; /* Declare that framework can not use the data in pTimeOut. */
    }
    else {
        if(((controlFlags & MPEGS_CONCEAL) && !(initControlFlags & MPEGS_CONCEAL)) ||
           (pMpegSurroundDecoder->pSpatialDec->errInt != MPS_OK)) { /* Account for errors that occured in
    ...
    if((err == MPS_OK) && !(controlFlags & MPEGS_BYPASSMODE) &&
       !(pMpegSurroundDecoder->upmixType == UPMIX_TYPE_BYPASS)) {
        SpatialDecChannelProperties(pMpegSurroundDecoder->pSpatialDec, channelType, channelIndices, mapDescr);
    }
...
bail:
    ...
    if(newData) {
        ...
        pMpegSurroundDecoder->pSpatialDec->numParameterSetsPrev =
            pMpegSurroundDecoder->bsFrames[pMpegSurroundDecoder->bsFrameDecode].numParameterSets;
    }
...
void mpegSurroundDecoder_Close(CMpegSurroundDecoder *pMpegSurroundDecoder) {
    if(pMpegSurroundDecoder != NULL) {
        ...
        for(int32_t i = 0; i < 1; i++) { SpatialDecCloseBsFrame(&pMpegSurroundDecoder->bsFrames[i]); }
...
SACDEC_ERROR mpegSurroundDecoder_SetParam(CMpegSurroundDecoder *pMpegSurroundDecoder, const SACDEC_PARAM param,
                                          const int32_t value) {
    SACDEC_ERROR      err = MPS_OK;
    ...
    switch(param) {
        case SACDEC_OUTPUT_MODE:
            ...
            if(err == MPS_OK) {
                if(0) { err = MPS_INVALID_PARAMETER; }
        ...
        case SACDEC_INTERFACE:
            if(value < 0 || value > 1) { err = MPS_INVALID_PARAMETER; }
            if(err != MPS_OK) { goto bail; }
        ...
        case SACDEC_BS_INTERRUPTION:
            if((err == MPS_OK) && (value != 0)) {
                updateMpegSurroundDecoderStatus(pMpegSurroundDecoder, MPEGS_INIT_BS_INTERRUPTION, MPEGS_SYNC_LOST,
                                                MPEGS_STOP);
            }
            break;
        ...
        case SACDEC_CLEAR_HISTORY:
            if((err == MPS_OK) && (value != 0)) {
                ...
                updateMpegSurroundDecoderStatus(pMpegSurroundDecoder, MPEGS_INIT_CLEAR_HISTORY, MPEGS_SYNC_LOST,
                                                MPEGS_STOP);
            }
            break;
        ...
        case SACDEC_CONCEAL_NUM_KEEP_FRAMES:
            ...
            if(err != MPS_OK) { goto bail; }
            if(pUserParams->concealNumKeepFrames != (uint32_t)value) {
                pUserParams->concealNumKeepFrames = (uint32_t)value;
                pMpegSurroundDecoder->initFlags[pMpegSurroundDecoder->bsFrameDecode] |=
        ...
        case SACDEC_CONCEAL_FADE_OUT_SLOPE_LENGTH:
            ...
            if(err != MPS_OK) { goto bail; }
            if(pUserParams->concealFadeOutSlopeLength != (uint32_t)value) {
                pUserParams->concealFadeOutSlopeLength = (uint32_t)value;
                pMpegSurroundDecoder->initFlags[pMpegSurroundDecoder->bsFrameDecode] |=
                    MPEGS_INIT_CHANGE_CONCEAL_PARAMS;
            }
            break;
        ...
        case SACDEC_CONCEAL_FADE_IN_SLOPE_LENGTH:
            ...
            if(err != MPS_OK) { goto bail; }
            if(pUserParams->concealFadeInSlopeLength != (uint32_t)value) {
                pUserParams->concealFadeInSlopeLength = (uint32_t)value;
                pMpegSurroundDecoder->initFlags[pMpegSurroundDecoder->bsFrameDecode] |=
                    MPEGS_INIT_CHANGE_CONCEAL_PARAMS;
            }
            break;
        ...
        case SACDEC_CONCEAL_NUM_RELEASE_FRAMES:
            ...
            if(err != MPS_OK) { goto bail; }
            if(pUserParams->concealNumReleaseFrames != (uint32_t)value) {
                pUserParams->concealNumReleaseFrames = (uint32_t)value;
                pMpegSurroundDecoder->initFlags[pMpegSurroundDecoder->bsFrameDecode] |=
                    MPEGS_INIT_CHANGE_CONCEAL_PARAMS;
            }
            break;
...
SACDEC_ERROR mpegSurroundDecoder_IsPseudoLR(CMpegSurroundDecoder *pMpegSurroundDecoder, int32_t *bsPseudoLr) {
    if(pMpegSurroundDecoder != NULL) {
        const SPATIAL_SPECIFIC_CONFIG *sscDecode =
            &pMpegSurroundDecoder->spatialSpecificConfig[pMpegSurroundDecoder->bsFrameDecode];
        *bsPseudoLr = (int32_t)sscDecode->bsPseudoLr;
        return MPS_OK;
    }
    else
    ...
    if(self != NULL) {
        ...
        if(coreCodec > AOT_NULL_OBJECT) {
            if(IS_LOWDELAY(coreCodec)) {
                ...
                if(self->mpegSurroundUseTimeInterface) { outputDelay += 320 + 384; /* QMF and hybrid analysis */ }
            }
        }
    }
